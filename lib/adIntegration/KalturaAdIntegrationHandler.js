...
KalturaAdIntegrationHandler = {
	...
	getAdMediaFiles : function(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, downloadCallback, successCallback, errorCallback) {
		
		response.log('Parsing ads from [' + cuePoint.sourceUrl + ']');
		...
		if(cachedUrl == null){
			KalturaUrlTokenMapper.mapFixedTokens(request, response, cuePoint, entry, metadata, playerConfig, function(cachedUrl){
		...
		var doGetAdMediaFiles = function(evaluatedUrl){
			...
			KalturaVastParser.parse(evaluatedUrl, headers, function(adServerResponse) {
				...
				for(var adFileId in adPerAspectRatio){
					...
					response.log('Selected ad: url [' + adFileInfo.fileURL + '], downloading to [' + adFileInfo.sharedFilePath + ']');
	...
	selectMediaFilePerAspectRatio : function(ad, mediaInfos) {
		...
		var getBestMediaInfo = function(groupEncodingIds){
			...
			response.log('Best media info: bitrate [' + selectedMediaInfo.video.bitrate + '], width [' + selectedMediaInfo.video.width + '], height [' + selectedMediaInfo.video.height + ']');
		...
		for(aspectRatioGroup in originalAssetsAspectRatioGroups){
			...
			if (mediaFile) {
				...
				if(adFileId in adPerAspectRatio){
					...
					response.log('Added encoding ids to media file: [' + adPerAspectRatio[adFileId].fileURL + '] encoding ids [' + adPerAspectRatio[adFileId].encodingIds + ']');
				}
				else{
					var adFileInfo = {
						...
						response.log('Selected media file: [' + adFileInfo.fileURL + '] for encoding ids [' + adFileInfo.encodingIds + ']');
				}								
			}		
		}
	...
	sendBeacon: function(trackingId, segmentIndex, outputStart, outputEnd){
		...
		response.log('start sendBeacon for trackingId: [' + trackingId + '] segmentIndex: [' + segmentIndex + '] outputStart: [' + outputStart + '] outputEnd: [' + outputEnd + ']');
		var checkBeaconProgress = function(progressStartPercent, progressEndPercent, beaconPercent, eventType, trackingInfo){
			...
			if(trackingInfo.hasOwnProperty(eventType) && progressStartPercent >= beaconPercent && progressStartPercent < beaconPercent + 25){
				...
				response.log('sending beacons of type: [' + eventType + '] with timer: [' + timer + ']');
		...
		KalturaCache.get(trackingId, function(trackingInfo){
			if(trackingInfo){
				...
				response.log('segmentIndex: [' + segmentIndex + '] progressStartPercent: [' + progressStartPercent + '] progressEndPercent: [' + progressEndPercent + ']');
			...
			response.log('Tracking info not found in cache for tracking id: [' + trackingId + ']: ' + err);

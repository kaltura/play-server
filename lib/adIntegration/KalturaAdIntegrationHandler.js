...
KalturaAdIntegrationHandler = {
	...
	getAdMediaFiles : function(cuePoint, entry, entryMetadata, playerConfig, headers, mediaInfos, successCallback, errorCallback) {
	...
	findBestMatch : function(adMediaFiles, mediaInfo) {
		...
		for (var mfIdx = 0; mfIdx < adMediaFiles.length; mfIdx++) {
			...
			if (!mediaFileCandidate)
				...
				var compareCurrentToMi = kalturaMediaInfo.compare(mediaInfo.video.bitrate, mediaInfo.video.width, mediaInfo.video.height, currentMediaFile.bitrate, currentMediaFile.width, currentMediaFile.height);
				var compareCurrentToCandidate = kalturaMediaInfo.compare(mediaFileCandidate.bitrate, mediaFileCandidate.width, mediaFileCandidate.height, currentMediaFile.bitrate, currentMediaFile.width, currentMediaFile.height);
				var compareCandidateToMi = kalturaMediaInfo.compare(mediaInfo.video.bitrate, mediaInfo.video.width, mediaInfo.video.height, mediaFileCandidate.bitrate, mediaFileCandidate.width, mediaFileCandidate.height);

...
require('../managers/KalturaMediaManager');
...
KalturaAdIntegrationHandler = {
	...
	getAdMediaFiles : function(cuePoint, entry, entryMetadata, playerConfig, headers, mediaInfos, sessionId, downloadCallback, successCallback, errorCallback) {
		
		KalturaLogger.log('Parsing ads from [' + cuePoint.sourceUrl + ']');

		KalturaUrlTokenMapper.mapTokens(cuePoint.sourceUrl, cuePoint.id, entry, entryMetadata, playerConfig, function(evaluatedUrl){
			var filesCount = 0;
			...
			KalturaVastParser.parse(evaluatedUrl, headers, function(response) {
				...
				if(ad.ad != null && ad.creative != null){
					...
					trackingInfo.duration = ad.creative.duration * 1000; //save in milliseconds
	...
	//TODO: do i need chunk or ts size? what units - millisec ?
	sendBeacon: function(trackingId, chunkType, chunkStartOffset, chunkSize){
		
		var sendBeaconForType = function(events, timer){
			var sleep = function() {
			    setTimeout(function(){ sendBeaconForType(events); }, timer);
			};
			
			if(timer > 0)
				sleep();
			for(var i=0; i < events.length; i++){
				var url = events[i].trim();
				KalturaLogger.log('sending beacon : [' + url + ']');
				http.get(url, function(res){
					KalturaLogger.log('beacon sent with status: [' + res.statusCode + ']');
		...
		KalturaCache.get(trackingId, function(trackingInfo){
			if(trackingInfo){
				...
				var progressStartPercent = chunkStartOffset / trackingInfo.duration * 100;
				var progressEndPercent = (chunkStartOffset + chunkSize) / trackingInfo.duration * 100;
				...
				switch (chunkType) {
				case KalturaMediaManager.STATE_PRE_AD:
				...
				case KalturaMediaManager.STATE_PAD:
				...
				case KalturaMediaManager.STATE_POST_AD:
					if('complete' in trackingInfo){
						sendBeaconForType(trackingInfo.complete);						
						delete trackingInfo.complete;						
					}

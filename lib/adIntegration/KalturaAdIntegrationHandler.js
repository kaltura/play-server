...
KalturaAdIntegrationHandler = {
	...
	getAdMediaFiles : function(cuePoint, entry, entryMetadata, playerConfig, headers, mediaInfos, successCallback, errorCallback) {
		...
		KalturaVastParser.parse(evaluatedUrl, headers, function(response) {
			...
			for(var aspectRatioGroup in adPerAspectRatio){
				var adInfo = adPerAspectRatio[aspectRatioGroup];
...
//TODO
	...
	selectAdAndMediaFilePerAspectRatio : function(response, duration, mediaInfos) {
		...
		var getBestMediaInfo = function(groupEncodingIds){
			...
			for (var encodingId in groupEncodingIds) {
		...
		for(aspectRatioGroup in originalAssetsAspectRatioGroups){
			...
			mediaFile = KalturaAdIntegrationHandler.findBestAdFile(ad.creative.mediaFiles, bestOriginalAsset);
			if (mediaFile) {
				var adInfo = {
				...
				adPerAspectRatio[aspectRatioGroup] = adInfo;
			}		
		}

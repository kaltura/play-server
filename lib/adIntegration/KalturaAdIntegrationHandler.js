...
KalturaAdIntegrationHandler = {
	...
	sendBeacon: function(trackingId, adSequence, totalDuration, segmentIndex, outputStart, outputEnd){
		...
		KalturaCache.get(trackingId, function(trackingInfos){
			for(var i=0; i < adSequence.length; i++){
				if(trackingInfos && trackingInfos[i]){
					KalturaLogger.log('Tracking info found in cache for tracking id: [' + trackingId + '] and sequence: [' + i + ']');
					var trackingInfo = trackingInfos[i];
										
					var progressStartPercent = outputStart / totalDuration * 100;
					var progressEndPercent = outputEnd / (totalDuration + trackingInfos[i].duration) * 100;
					...
					totalDuration += trackingInfos[i].duration;	
				}
				else{

		...
		var sendBeaconForType = function(events){
			var httpGet = function(url){
				http.get(url, function(res){
					KalturaLogger.log('beacon for tracking id [' + trackingId + '] url [' + url + '] sent with status: [' + res.statusCode + ']');
					res.on('data', function() { /* do nothing */ });
				}).on('error', function(e){
					KalturaLogger.error('Failed to send beacon for tracking id [' + trackingId + '] url [' + url + '], ' + e.message);
				});							
			};

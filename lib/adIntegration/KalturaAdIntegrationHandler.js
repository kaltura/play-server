...
KalturaAdIntegrationHandler = {
	...
	getAdMediaFiles : function(request, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, downloadCallback, errorCallback) {
			...
			KalturaVastParser.parse(evaluatedUrl, headers, function(adServerResponse) {
				...
				if(ads.length > 0 ){
					for (var adIdx = 0, adLen = adPod.length; adIdx < adLen; adIdx++) {
						KalturaAdIntegrationHandler.processSelectedAd(adPod[adIdx]);
					}
		...
		var processSelectedAd = function(ad){
	...
	sendBeacon: function(trackingId, adSequence, segmentIndex, outputStart, outputEnd){
		...
		KalturaCache.get(trackingId, function(trackingInfos){
			if(trackingInfos && trackingInfos[adSequence]){
				var trackingInfo = trackingInfos[adSequence];
				var totalPlayedDuration = 0;
				for(var seq = 1; seq < adSequence; seq++){
					totalPlayedDuration += trackingInfos[seq].duration;
				}
				KalturaLogger.log('Tracking info found in cache for tracking id: [' + trackingId + ']');	
				var progressStartPercent = outputStart / (totalPlayedDuration + trackingInfo.duration) * 100;
				var progressEndPercent = outputEnd / (totalPlayedDuration + trackingInfo.duration) * 100;

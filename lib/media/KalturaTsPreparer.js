...
var KalturaTsPreparer = {
    ...
    cutTsFiles: function(ffmpegBin, ffprobeBin, cutOffset, leftPortion, inputFiles, callback) {
    	...
    	KalturaTsPreparer.readFilesAndGetFrames(ffprobeBin, inputFiles, function(error, inputBuffers, framesInfo) {
    		...
    		cutDetails = tsPreparer.getCutDetails(inputBuffers.buffers, framesInfo, cutOffset, leftPortion);
    		...
    		KalturaTsPreparer.clipWithFfmpeg(ffmpegBin, ffprobeBin, boundedSection, cutOffsetSec, leftPortion, function(error, clippedBuffers, clippedFramesInfo) {
    			...
    			tsPreparer.fixTimestamps(clippedBuffers.buffers[0], clippedFramesInfo, cutDetails.timestamps, leftPortion);
    
    			// add the clip to the output buffers and fix the continuity counters
    			if (leftPortion) {
    				outputBuffers = outputBuffers.concat(clippedBuffers);
    
    				tsPreparer.fixContinuityForward(outputBuffers.buffers);
    			} else {
    				outputBuffers = clippedBuffers.concat(outputBuffers);
    
    				tsPreparer.fixContinuityBackward(outputBuffers.buffers);
    			}
    
    			// write the result
				KalturaTsPreparer.logger.log('Writing ' + outputBuffers);

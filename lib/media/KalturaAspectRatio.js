KalturaAspectRatio = {
	...
	convertFrameSizeForGroups: function(width, height, aspectRatioGroups){
		...
		for(var group in this.aspectRatioGroups){
			if(!aspectRatioGroups.hasOwnProperty(group)){
				continue;
			}
			if(prevGroup == null){
				prevGroup = group;
		...
		else if (ratio < lowerLimit && aspectRatioGroups.hasOwnProperty('LOW')){
			return 'LOW';
		}			
		else if (ratio > upperLimit && aspectRatioGroups.hasOwnProperty('HIGH')){
			return 'HIGH';
		}			
	...
	getBestAspectRatioForGroup: function(originalGroup, aspectRatioGroups){
		...
		for(var i=0; i<aspectRatioGroups.length;i++){

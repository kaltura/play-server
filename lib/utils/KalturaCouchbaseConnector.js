...
let KalturaEncryption = require('../utils/KalturaEncryption');
...
class KalturaCouchbaseConnector {
	...
	get(key, callback, errorCallback, is_encrypted)
	{
		...
		let getPromise = new Promise(function(resolve,reject) {
			This._bucket.get(key, namespace.bind(function (err, result) {
				if (err)
				{
				...
				else
				{
					if (is_encrypted)
					{
						KalturaLogger.debug("Couchbase.get using encrypted path");
	...
	set(key, value, lifetime, callback, errorCallback, is_encrypted)
	{
		...
		let upsertPromise =  new Promise(
			function(resolve, reject)
			{
				This._bucket.upsert(key, value, {expiry: lifetime},
					namespace.bind(function (err, result) {
						if (err)
						{
	...
	getMulti(keys, callback, errorCallback, is_encrypted)
	{
		...
		let getMultiPromise = new Promise(function (resolve, reject) {
			This._bucket.getMulti(keys, namespace.bind(function (err, results) {
				let resultsValues = [];
				if (err && err != "1")
				{
	...
	touch(key, lifetime, callback, errorCallback)
	{
		...
		if (parseInt(this._config.touchEnabled))
		{
			let touchPromise = new Promise(function (resolve, reject) {
				This._bucket.touch(key, lifetime, namespace.bind(function (err, result) {
					if (err)
					{
	...
	add(key, value, lifetime, callback, errorCallback, is_encrypted)
	{
		...
		let insertPromise = new Promise(function (resolve, reject)
		{
			...
			This._bucket.insert(key, value, {"expiry": lifetime}, namespace.bind(function (err, result)
			{
				if (err)
				{
	...
	append(key, value, callback, errorCallback, is_encrypted)
	{
		...
		let appendPromise =  new Promise(function(resolve, reject) {
			This._bucket.append(key, value, namespace.bind(function (err, result) {
				if (err)
				{
	...
	del(key, callback, errorCallback)
	{
		...
		let delPromise =  new Promise(function(resolve, reject) {
			This._bucket.remove(key, namespace.bind(function (err, result) {
				if (err)
				{
	...
	replace(key, value, lifetime, callback, errorCallback, is_encrypted)
	{
		...
		let delPromise =  new Promise(function(resolve, reject) {
			This._bucket.remove(key, value, {expiry: lifetime}, namespace.bind(function (err, result) {
				if (err)
				{
	...
	touchMulti(keys, lifetime, callback, errorCallback)
	{
		let This = this;
		const namespace = this._namespace();
		...
		keys.forEach(function(key, index, array){
			This._bucket.touch(key,lifetime, namespace.bind(function(err, data){
				if(err){
					var errMessage = 'Couchbase.touchMulti [' + key + ']:' + err;
					if(errorCallback){
					...
					KalturaLogger.debug('Couchbase.touchMulti [' + key + ']: OK', stackSource);
					missingAnswers--;
	...
	_decryptValues(object, encKey, iv)
	{
		...
		if (typeof object == 'string')
			return KalturaEncryption.prototype.decrypt(object, encKey, iv);
		else
		{
	...
	_encryptValues(object, encKey, iv)
	{
		let This = this;
		if (typeof object == 'string')
			return KalturaEncryption.prototype.encrypt(object, encKey, {iv:iv});
		else
		{
			for (let key in object) {
				let val = object[key];

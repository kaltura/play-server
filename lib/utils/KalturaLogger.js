...
var KalturaOutputWriter = function(logDir, fileName, udgName, useLogCompressor) {
	this.fileName = null;
	this.file = null;
	this.udg = null;
	if (useLogCompressor && parseInt(useLogCompressor) && udgName) {
		this.udg = unix.createSocket('unix_dgram');
                this.bindUdgSocket(this.udg, logDir + '/' + udgName);
		...
		console.log('Error initialize log output writer for [' + fileName + '/' + udgName + ']');
	}
};

KalturaOutputWriter.prototype.print = function() {
...
KalturaLogger = {
	...
	init: function(){
		...
		if(KalturaLogger.config.accessResponseHeaders){
        	...
        	KalturaLogger.logOutputWriter = 
			new KalturaOutputWriter(KalturaLogger.config.logDir, KalturaLogger.config.logName, KalturaLogger.config.logUdgName, KalturaLogger.config.useLogCompressor);
		KalturaLogger.errorOutputWriter = 
			new KalturaOutputWriter(KalturaLogger.config.logDir, KalturaLogger.config.errorLogName, KalturaLogger.config.errorUdgName, KalturaLogger.config.useLogCompressor);
		KalturaLogger.accessOutputWriter = 
			new KalturaOutputWriter(KalturaLogger.config.logDir, KalturaLogger.config.accessLogName, KalturaLogger.config.accessUdgName, KalturaLogger.config.useLogCompressor);
	},

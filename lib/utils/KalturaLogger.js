	...
	access: function(request, response){
		...
		var responseWrite = response.write;
                response.write = function(data) {
                        responseSize = responseSize + data.toString().length;
		...
		response.end = function(body){
			...
			if (body)
                                responseSize = responseSize + body.toString().length;
			...
			logLine += ' ' + Math.floor(executionTime / 1000) + '/' + (executionTime * 1000) + ' ' + responseSize;

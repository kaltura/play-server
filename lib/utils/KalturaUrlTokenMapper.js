...
KalturaUrlTokenMapper = {
	...
	mapTokensVod: function(request, cuePointId, cuePointUrl, partnerId, entryId, callback) {
		...
		let This = this;
		this.getEntryAndMetadata(partnerId, entryId, metadataProfileId,
			function(entryObj, metadataObj){
				if (!entryObj)
					return callback(cuePointUrl);
				This.doMapFixedTokens(request, cuePointId, cuePointUrl, entryObj, metadataObj, null,
	...
	getEntryAndMetadata : function(partnerId, entryId, metadataProfileId, callback) {
		...
		if (metadataProfileId)
		{
			...
			promises.push(apiConnector.handleApiRequest('metadata', 'listAction', [filter], partnerId).
				then((data) => metadata = data.objects[0].xml));
		}

		Promise.all(promises).then(
			function () {
				if (metadata)
					parseString(metadata, function (err, metadata){
						if (err) {
							KalturaLogger.error("Can't Parsed metadata ");
							return callback();
						}

...
KalturaUrlTokenMapper = {
	...
	mapTokens: function(sourceUrl, cuePointId, entry, entryMetadata, playerConfig){
		var res = sourceUrl;
		...
		KalturaCache.get(cuePointUrlKey, function(cachedUrl){
			if(cachedUrl){
				KalturaCache.touch(cuePointUrlKey, KalturaConfig.config.cache.adInfo);
				res = doMapTokens(cachedUrl, 'dynamic');
			}
			else{
				KalturaLogger.log('Cue point url not found in cache');
				res = doMapTokens(sourceUrl, 'fixed');				
				KalturaCache.set(cuePointUrlKey, res, KalturaConfig.config.cache.adInfo);

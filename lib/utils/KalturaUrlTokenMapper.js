...
KalturaUrlTokenMapper = {
	tokenAttributeToUtilityMap :{
		'{please_put_dynamic_page_url}': '{utility.referrer}',
		'{random_number}': '{utility.random}',
	},
	...
	mapDynamicTokens: function(request, cachedUrl, playerConfig){
		...
		if( request != null){
			data.mediaSession = {};
			data.mediaSession.userIPaddress = request.ip;
		...
		for(var attributeName in queryString){
			var valueToEveluate = queryString[attributeName];
			if(this.tokenAttributeToUtilityMap[valueToEveluate]){
				valueToEveluate = this.tokenAttributeToUtilityMap[valueToEveluate];
			}
			var attributeValue = KalturaUrlTokenMapper.evaluate(data, valueToEveluate);
	...
	mapFixedTokens: function(request, cuePoint, entry, metadata, playerConfig, callback){
		var cachedUrl = cuePoint.sourceUrl;
		
		if (typeof cachedUrl == 'string' && cachedUrl.indexOf('[') > -1) {
             KalturaLogger.log("Yossilog:: switching chached url tags");
             cachedUrl = cachedUrl.replace(/\[/gi, '{');
             cachedUrl = cachedUrl.replace(/\]/gi, '}');
		}
		...
		for(var attributeName in queryString){
			...
			KalturaLogger.log("Evaluated attribute: [" + attributeName + "] value: [" + attributeValue + "]");
			if(attributeValue.indexOf(defaultTokenValue) < 0)					
				queryString[attributeName] = attributeValue;
		}
		
		parsedUrl.search = _qs.stringify(queryString);
		parsedUrl.query = queryString;
		cachedUrl =  _url.format(parsedUrl);

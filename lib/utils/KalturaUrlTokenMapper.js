...
KalturaUrlTokenMapper = {
	...
	mapFixedTokens: function(request, response, cuePoint, entry, metadata, playerConfig, callback){
		var cachedUrl = cuePoint.sourceUrl;
		var cuePointUrlKey = KalturaCache.getCuePointUrl(cuePoint.id);
		var data = {};
		var parsedUrl = _url.parse(cuePoint.sourceUrl, true);
		var queryString = parsedUrl.query;		
		...
		response.log('Saving cue point url in cache: [' + cachedUrl + ']');

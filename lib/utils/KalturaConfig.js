const util = require('util');
var os = require('os');
var fs = require('fs');
var ini = require('node-ini');

KalturaConfig = {
	config: null,
	configFiles: {},
	
	init: function(){
		if(process.argv.length > 2 && !this.isTestingMode() ){
			this.config = JSON.parse(process.argv[2]);
		}
		else{
			process.chdir(__dirname);
			var configDir = '../../config';
			var cacheDir = '../../cache';
	
			if (!fs.existsSync(cacheDir)) {
				fs.mkdirSync(cacheDir);
			}
	
			var files = fs.readdirSync(configDir);
			var This = this;
	
			var configData = '';
			var pattern = /.+\.ini$/;
			for ( var index in files) {
				if(!pattern.test(files[index]))
					continue;
				
				var filePath = configDir + '/' + files[index];
				configData += os.EOL;
				configData += fs.readFileSync(filePath, 'utf-8');
	
				fs.lstat(filePath, function(err, stats) {
					if (err) {
						throw new Error(`Failed to init KalturaConfig due to ${util.inspect(err)}`);
					} else {
						This.configFiles[filePath] = stats.mtime;
					}
				});
			}
	
			var cacheConfigPath = cacheDir + '/config.ini';
			fs.writeFileSync(cacheConfigPath, configData);
	
			this.config = ini.parseSync(cacheConfigPath);
		}
	},

	isTestingMode: function(){
		if (process.argv[1].indexOf('mocha') !== -1 || process.argv[1].toLowerCase().indexOf('test') !== -1)
			return true;
	},

	watchFiles: function(callback) {
		setInterval(function(){
			var handled = false;
			for(var filePath in this.configFiles){
				fs.lstat(filePath, function(err, stats) {
					if(handled){
						return;
					}
					if (err) {
						throw new Error(`Failed to watch files of KalturaConfig due to ${util.inspect(err)}`);
					} else {
						if(this.configFiles[filePath] < stats.mtime){
							handled = true;
							KalturaConfig.init();
							callback();
						}
					}
				});
			}
		}, 30000);
	}
};
KalturaConfig.init();


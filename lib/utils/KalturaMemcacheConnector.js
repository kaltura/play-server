...
KalturaMemcache = {
	...
	touchMulti : function(keys, lifetime, callback, errorCallback) {
		...
		var This = this;
		keys.forEach(function(key, index, array){
			This.server.touch(key,lifetime, namespace.bind(function(err, data){
				if(err){
					var errMessage = 'Cache.touchMulti [' + key + ']:' + err;
					if(errorCallback){
						errorCallback(errMessage);
					}
					else{
					...
					KalturaLogger.debug('Cache.touchMulti [' + key + ']: OK', stackSource);
					missingAnswers--;
					if(!missingAnswers && callback)
						callback();
				}

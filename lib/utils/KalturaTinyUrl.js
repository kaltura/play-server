...
class KalturaTinyUrl
{
    ...
    static insert(url,callback=null)
    {
        ...
        KalturaCache.set(	tinyUrl,
							url,
							ttl,
							callback,
							(err)=>
							{
								KalturaCache.touch(tinyUrl,
															ttl,
															callback,
															(err)=> {throw (`Fail to touch cache key:${tinyUrl} err:${err}`)});
							}
						);
        return tinyUrl;
    }

    static load(tinyUrl,callback,errCallback=null)
    {
        KalturaCache.get(tinyUrl,
						(value)=>
						{
							...
							if(!value || value=='' || value==null)
								throw (`Key ${tinyUrl} has no value in cache`);
							callback(value);
						},
						(err)=>	errCallback(err)
						);

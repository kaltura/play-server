...
class KalturaTinyUrl
{
	...
	static insert(url)
	{
		...
		KalturaCache.set(tinyUrl, url, ttl,
			() => KalturaLogger.debug(`Managed to save tinyUrl: ${tinyUrl} for url ${util.inspect(url)} for ${ttl} seconds in cache`),
	...
	static load(tinyUrl, callback, errCallback)
	{
		KalturaCache.get(tinyUrl,
			function (value)
			{
				KalturaLogger.debug(`Return value from tiny url cache is: ${util.inspect(value)}`);
				if (!value)

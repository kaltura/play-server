...
if (typeof String.prototype.md5 != 'function') {
	String.prototype.md5 = function() {
		return KalturaUtils.getShortHashKey(new Buffer(this.toString()));
	};
}
...
if (typeof Buffer.prototype.md5 != 'function') {
	Buffer.prototype.md5 = function() {
		return KalturaUtils.getShortHashKey(this);
	};
}
...
KalturaUtils = {
	...
	validateErrorStatus : function(statusCode, errorCallback, urlStr){
		if(statusCode != 200){
			if(errorCallback)
				return errorCallback('Invalid http status: ' + statusCode + ' while trying to fetch ' + urlStr);
			else
				KalturaLogger.error('Invalid http status: ' + statusCode + ' while trying to fetch ' + urlStr);
		}
	},

	fixHostSyntax: function(host)
	{
		if (host.indexOf(':') !== -1)
	...
	fixHostHeader: function(headers)
	{
		if (headers.host)
			headers.host = this.fixHostSyntax(headers.host);
		else if (headers.HOST)
			headers.HOST = this.fixHostSyntax(headers.HOST);
		else if (headers.Host)
			headers.Host = this.fixHostSyntax(headers.Host);
		else if (headers.hostname)
			headers.hostname = this.fixHostSyntax(headers.hostname);
	},
	...
	getHttpUrl : function(urlStr, headers, successCallback, errorCallback) {
		...
		var request = httpModule.request(options, function(response) {
			...
			response.on('end', function() {
				KalturaUtils.validateErrorStatus(response.statusCode, errorCallback, urlStr);
				if(successCallback){
	...
	md5OnFile : function(urlStr, headers, bytes, successCallback, errorCallback) {
		...
		var request = httpModule.request(options, namespace.bind(function(response) {
	...
	downloadHttpUrl : function(urlStr, options, successCallback, errorCallback) {
		...
		fs.exists(options.filePath, function(exists){
			if(exists){
				...
				// first thing - we answer in success and next we set a validation to make sure that the sizes are correct
				var parsedUrl = url.parse(urlStr);
				var httpModule = KalturaUtils.getHttpModuleByProtocol(parsedUrl.protocol);
				var getHeadersOptions = {
					hostname: parsedUrl.hostname,
					port: parsedUrl.port,
					path: parsedUrl.path,
					method: 'HEAD'
				}
				const req = httpModule.request(getHeadersOptions, function(res) {
					var lenInBytes = parseInt(res.headers['content-length'], 10);
					var stats = fs.statSync(options.filePath);
					var fileSizeInBytes = stats.size;
					if (Math.abs(fileSizeInBytes - lenInBytes) > 10)
					{
						KalturaLogger.error(`URL marked for download seems to exist on disk yet size [${fileSizeInBytes}] is different from content-length [${lenInBytes}] , re-trying`);
	...
	getShortHashKey: function(longKey)
	{
		return crypto.createHash('md5').update(longKey).digest('hex');
	},

	hashJsonObject: function(object)
	{
		return util.inspect(object).replace(/\}|\{|\[|\]|\"|\'|\,|\:| /g,'').md5();
	}

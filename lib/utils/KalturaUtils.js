...
KalturaUtils = {
	...
	getHttpUrlByteLimit : function(urlStr, headers, bytes, successCallback, errorCallback) {
		KalturaLogger.log('Start download [' + urlStr + '] bytes [' + bytes + ']');
		var parsedUrl = url.parse(urlStr);
		var httpModule = KalturaUtils.getHttpModuleByProtocol(parsedUrl.protocol);
		var options = {
			hostname : parsedUrl.hostname,
		...
		var request = httpModule.request(options, function(response) {
			...
			response.on('data', function(data) {
				  ...
				  KalturaLogger.debug('current length=' + length);
			      if (length >= bytes){
			        response.destroy();
			      }
			});
			response.on('end', function() {
				...
				if(successCallback){
					KalturaLogger.log('Downloaded [' + length + '] bytes');
					 var bytesBuffer = Buffer.concat(dataBuffers, length);
					successCallback(bytesBuffer.slice(0, bytes));
				}
			});
		});

...
KalturaUtils = {
	...
	downloadHttpUrl : function(urlStr, options, successCallback, errorCallback, recursion = false) {
		...
		fs.exists(options.filePath, function(exists){
			if(exists){
				...
				options.method = 'GET';
				httpModule.request(options, function(res) {
					...
					else
					{
						...
						if (!recursion)
							this.downloadHttpUrl(urlStr, options, successCallback, errorCallback, true);
						else
							errorCallback(`URL marked for download seems to exist on disk yet size [${fileSizeInBytes}] is different from content-length [${lenInBytes}] , already ran recursion once`);
					}
				});
			}
			else{

...
//		});
	...
	removeRenditionIdFromEntryRequiredValue: function(entryRequired, renditionId, retries){
		var This = this;
		var lockKey = 'Lock' + entryRequired;
		this.add(lockKey, 1, KalturaConfig.config.cache.entryRequired, function() { // add fails if key exists
			KalturaLogger.debug("Removing " + renditionId + " from " + entryRequired + " values");

			This.get(entryRequired, function (data) {
				var currentRenditions = This.extractEntryRequiredValue(data);
				var newEntryRequired = "\n";
				var shouldChange = false;
				var foundValidRendition = false;
				for (var i = 0; i < currentRenditions.length; i++) {
					if (currentRenditions[i].indexOf(renditionId) == -1) {
						if (currentRenditions[i].trim().length !== 0) {
							foundValidRendition = true;
						}
						newEntryRequired = newEntryRequired.concat("\n").concat(currentRenditions[i].trim());
					} else {
						shouldChange = true;
					}
				}
				if (shouldChange) {
					if (foundValidRendition) {
						This.set(entryRequired, newEntryRequired, KalturaConfig.config.cache.entryRequired,
							...
							function () {
								...
								setTimeout(function () {
									This.removeRenditionIdFromEntryRequiredValue(entryRequired, renditionId);
								}, 100);
							});
					} else {
			...
			if (!retries)
				retries = 0;
			if (retries < 10)
				setTimeout(function(){This.removeRenditionIdFromEntryRequiredValue(entryRequired, renditionId, retries++);}, 100);
		});

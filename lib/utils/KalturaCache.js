...
KalturaCache = {
	...
	init: function() {
		...
		this.cacheInstance = KalturaCacheManager.getInstance(KalturaConfig.config.cache.type);
	},
	...
	VOD_SESSION_ADS_READY_KEY_PREFIX: 'adsReady',
	...
	removeRenditionIdFromEntryRequiredValue: function(entryRequired, renditionId, retries){
		...
		this.add(lockKey, 1, lockTimeout, function() { // add fails if key exists
			...
			KalturaLogger.debug("Failed to get lock for deleting rendition " + renditionId + " from entryRequired : " + entryRequired + " retrying , reason :" + err);
			if (!retries)
				retries = 0;
	...
	get : function(key, callback, errorCallback, is_encrypted=false)
	{
		this.cacheInstance.get(key, callback, errorCallback, is_encrypted);
	},

	set : function(key, value, lifetime, callback, errorCallback, is_encrypted=false)
	{
	...
	add : function(key, value, lifetime, callback, errorCallback, is_encrypted=false)
	{
		this.cacheInstance.add(key, value, lifetime, callback, errorCallback, is_encrypted );
	},

	append : function(key, value, callback, errorCallback, is_encrypted=false)
	{
		this.cacheInstance.append(key, value, callback, errorCallback, is_encrypted );
	},

	getMulti : function(keys, callback, errorCallback, is_encrypted=false)
	{
	...
	replace : function(key, value, lifetime, callback, errorCallback, is_encrypted=false)
	{

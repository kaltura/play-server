...
KalturaCache = {
	...
	init: function(){
		...
		this.backupServer = new memcached(this.config.backupHostname + ':' + this.config.port, this.config);
		this.backupServer.on('failure', function( details ){ KalturaLogger.error( "Server " + details.server + " went down due to: " +   details.messages.join( '' ) ) });
		this.backupServer.on('issue',   function( details ){ KalturaLogger.error( "Server " + details.server + " encountered issues: " + details.messages.join( '' ) ) });
		...
		for (var i = 0; i < length; i++) {
			...
			var serverGet = new memcachedbin();
			serverGet.host = serverName;
			serverGet.port = this.config.binPort;
			serverGet.connect();

			this.binservers[i] = serverGet;
			var serverSet = memjs.Client.create(serverName + ':' + this.config.binPort,  binserverSetOptions);
			this.binserversSet[i] = serverSet;
		}
	...
	get : function(key, callback, errorCallback) {
		...
		This.server.get(key, function(err, data){
			if(err){
				...
				KalturaLogger.debug('Received empty response, retrying on other DC');
				This.backupServer.get(key, function(err, data){
					if(err){
						var errMessage = 'Cache.get on other DC [' + key + ']:' + err;
						if(errorCallback){
							errorCallback(errMessage);
						}
						else{
						...
						if (data)
							KalturaLogger.debug('Cache.get on other DC [' + key + ']: OK', stackSource);
						else
							KalturaLogger.debug('Cache.get on other DC [' + key + ']: Empty or not exist', stackSource);

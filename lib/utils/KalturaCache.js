...
KalturaCache = {
	...
	init: function(){
		...
		binserver.host = this.config.hostname;
		binserver.port = this.config.port;

		//TODO: start server async
//		binserver.on('connect', function() {
//			http.createServer(handleHttpRequest).listen(LOCAL_SERVER_PORT, '0.0.0.0');
//
//			KalturaLogger.log('Server running at http://0.0.0.0:' + LOCAL_SERVER_PORT + '/');
//		});
	...
	getServerAdId : function(cuePointId, renditionId) {
		return 'serverAdId-' + cuePointId + '-' + renditionId;
	},
	...
	getBinary : function(key, callback, errorCallback) {
		...
		this.serverbin.get(key, function(err, data){
			if(err){
				var errMessage = 'Cache.get [' + key + ']:' + err;
				if(errorCallback){
					errorCallback(errMessage);
	...
	getMultiBinary : function(keys, callback, errorCallback) {
		...
		keys.forEach(function(key, index, array){
			KalturaCache.serverbin.get(key, function(err, data){
    			if(err){

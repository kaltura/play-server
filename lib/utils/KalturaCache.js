...
KalturaCache = {
	...
	//KEY PREFIXES
	FFMPEG_QUEUE_COUNTER: 'ffmpegQueueCounter',
	FFMPEG_SLOT: 'ffmpegSlot',
	...
	incr : function(key, value, callback, errorCallback) {
		var stackSource = this.getStack();
		this.server.incr(key, value, function(err, data){
			if(err){
				var errMessage = 'Cache.incr [' + key + ']:' + err;
				if(errorCallback){
				...
				var errMessage = 'Cache.incr [' + key + '] does not exist';
				if(errorCallback){
					errorCallback(errMessage);
				}
				else{
				...
				KalturaLogger.debug('Cache.incr [' + key + ']: OK', stackSource);
				if(callback)
					callback(data);
			}
		});

...
//		});
	...
	removeRenditionIdFromEntryRequiredValue: function(entryRequired, renditionId){
		var currentRenditions = this.extractEntryRequiredValue(entryRequired);
		var newEntryRequired = "\n";
		var shouldChange = false;
		for(var i = 0; i < currentRenditions.length; i++){
			if(currentRenditions[i].indexOf(renditionId) == -1 ){
				newEntryRequired += "\n" + currentRenditions[i].trim();
			} else {
				shouldChange = true;
			}
		}
		if ( shouldChange ) {
			this.set(entryRequired, newEntryRequired);
		}
	},

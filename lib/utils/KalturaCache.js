...
KalturaCache = {
	...
	init: function(){
		...
		this.backupServer = new memcached(this.config.backup_hostname + ':' + this.config.port, this.config);
		...
		this.binserver.host = this.config.binHostname;
		this.binserver.port = this.config.binPort;
		...
		this.binserverSet = memjs.Client.create(this.config.binHostname + ':' + this.config.binPort,  binserverSetOptions);
	...
	getBinary : function(key, callback, errorCallback) {
		...
		KalturaCache.binserver.connect();

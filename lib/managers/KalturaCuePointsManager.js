 ...
 */
KalturaCuePointsManager.prototype.lastUpdatedAt = Math.round(Date.now()/1000)-7200;;
...
KalturaCuePointsManager.prototype.verifyEntryRequired = function(entryId){
	...
	var oldestSegmentTimeKey = KalturaCache.getKey(KalturaCache.OLDEST_SEGMENT_TIME_KEY_PREFIX, [entryId]);
	var cuePointsKey = KalturaCache.getKey(KalturaCache.ENTRY_CUE_POINTS_KEY_PREFIX, [entryId]);
	...
	KalturaCache.get(entryRequiredKey, function(data){
		if(!data){
			KalturaLogger.debug('Deleting cue points for entry ' + entryId);
			deleteCuePoint();
		}
		else{
			var watcherHandledKey = KalturaCache.getKey(KalturaCache.CUE_POINT_WATCHER_HANDLED_KEY_PREFIX, [entryId]);
			KalturaCache.set(watcherHandledKey, true, KalturaConfig.config.cache.watcherHandled);
					...
					if(oldestSegmentTime){
						for(var cuePointId in This.cuePoints[entryId].cuePoints){
							...
							if((cuePoint.startTime && oldestSegmentTime.offset > (cuePoint.startTime + cuePoint.duration)) || 
								...
								KalturaLogger.log('Deleting handled cue point from cache: [' + cuePointId + '] oldestSegmentTime [' + JSON.stringify(oldestSegmentTime) + ']');
...
KalturaCuePointsManager.prototype.handleCuePointsList = function(cuePointsList, filter, pager){
	...
	if(!cuePointsList){
		KalturaLogger.error('Client [cuePoint.list] system error');
	}
	else if(cuePointsList.objectType == 'KalturaAPIException'){

	...
	if(cuePointsList.objectType == 'KalturaAPIException'){
		...
		for(var i = 0; i < cuePointsList.objects.lemgth; i++){
			...
			if(!this.cuePoints[entryId]){
				this.cuePoints[entryId] = {};
			}
			this.cuePoints[entryId][cuePoint.id] = cuePoint;

			var cuePointsKey = this.cache.getCuePoints(entryId);
			this.add(cuePointsKey, this.cuePoints[entryId], 600, null, function(err){
				This.cache.replace(cuePointsKey, this.cuePoints[entryId], 600);
			});
		}
	}
};

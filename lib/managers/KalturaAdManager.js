		...
		if(exists){
			...
			if(KalturaConfig.config.ad.numFfmpegLowWatermark){
				maxFfmpegProcesses = parseInt(KalturaConfig.config.ad.numFfmpegLowWatermark);
            		...
            		var numFfmpegHighWatermark = 100;
			...
			ffmpegProcessesCmd.exec(function(numberOfProccesses){
				var numberOfFfmpeg = parseInt(numberOfProccesses.trim());
				if(numberOfFfmpeg > maxFfmpegProcesses) {
                    			KalturaLogger.error('Number of ffmpeg processes [' + numberOfProccesses + '] exceeded max [' + maxFfmpegProcesses + '], file [' + adPath + ']. Quitting.');
                		}
                		else if (maxFfmpegProcesses > numFfmpegHighWatermark) {
					var sleepHighWatermark = 20;
				    	...
				    	KalturaLogger.error('Number of ffmpeg processes [' + numberOfProccesses + '] exceeded high watermark [' + numFfmpegHighWatermark + '], file [' + adPath + ']. Retring in [' + sleepHighWatermark / 1000 + '] seconds.');
				    	setTimeout(This.exec, sleepHighWatermark * 1000, serverAdId, sourcePath, encodingParams);
				}
				else if (maxFfmpegProcesses > numFfmpegLowWatermark) {
					...
					KalturaLogger.error('Number of ffmpeg processes [' + numberOfProccesses + '] exceeded low watermark [' + numFfmpegLowWatermark + '], file [' + adPath + ']. Retring in [' + sleepLowWatermark / 1000 + '] seconds.');
					setTimeout(This.exec, sleepLowWatermark * 1000, serverAdId, sourcePath, encodingParams)
				}
				else{

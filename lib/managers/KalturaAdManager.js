	...
	fs.exists(adTsPath, function(exists){
		if(exists){
			...
			readTsFile(function(data){
				...
				kaltura.tsPreparer.savePreparedTsToMemcache(KalturaCache.getBinServerSetByKey(serverAdKey), serverAdKey, data, KalturaConfig.config.cache.adMedia, function(error){
					if(error){

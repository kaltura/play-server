...
KalturaAdManager.prototype.numOfProcesess = 0;
...
KalturaAdManager.prototype.exec = function(serverAdId, sourcePath, encodingParams){
	...
	fs.exists(adPath, function(exists){
		if(exists){
			...
			var numFfmpegLowWatermark = 1;
			if(KalturaConfig.config.ad.numFfmpegLowWatermark){
				maxFfmpegProcesses = parseInt(KalturaConfig.config.ad.numFfmpegLowWatermark);
			}
            		var numFfmpegHighWatermark = 5;
           		if(KalturaConfig.config.ad.numFfmpegHighWatermark){
                		maxFfmpegProcesses = parseInt(KalturaConfig.config.ad.numFfmpegHighWatermark);
            		}
 			...
 			var numberOfFfmpeg = This.numOfProcesess;
			if(numberOfFfmpeg > maxFfmpegProcesses) {
                    		KalturaLogger.error('Number of ffmpeg processes [' + numberOfFfmpeg + '] exceeded max [' + maxFfmpegProcesses + '], file [' + adPath + ']. Quitting.');
                	}
               		else if (numberOfFfmpeg  > numFfmpegHighWatermark) {
				var sleepHighWatermark = 20;
			    	if (KalturaConfig.config.ad.sleepHighWatermark) {
					sleepHighWatermark = parseInt(KalturaConfig.config.ad.sleepHighWatermark);
				}
				KalturaLogger.error('Number of ffmpeg processes [' + numberOfFfmpeg + '] exceeded high watermark [' + numFfmpegHighWatermark + '], file [' + adPath + ']. Retrying in [' + sleepHighWatermark + '] seconds.');
                    		setTimeout(function() {This.exec(serverAdId, sourcePath, encodingParams);}, sleepHighWatermark * 1000);
			}
			else if (numberOfFfmpeg  > numFfmpegLowWatermark) {
				var sleepLowWatermark = 2;
				if(KalturaConfig.config.ad.sleepLowWatermark){
					sleepLowWatermark = parseInt(KalturaConfig.config.ad.sleepLowWatermark);
				}
				KalturaLogger.error('Number of ffmpeg processes [' + numberOfFfmpeg + '] exceeded low watermark [' + numFfmpegLowWatermark + '], file [' + adPath + ']. Retrying in [' + sleepLowWatermark + '] seconds.');
                    		setTimeout(function() {This.exec(serverAdId, sourcePath, encodingParams);}, sleepLowWatermark * 1000);
			}
			else{
				KalturaLogger.log('Number of ffmpeg processes is [' + numberOfFfmpeg + '], transcoding file ['+ adPath + ']');
				...
				cmd = cmd.join(' ');
				This.numOfProcesess = This.numOfProcesess + 1;

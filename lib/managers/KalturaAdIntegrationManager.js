	...
	var sendBeaconForType = function(events, eventType){
		var httpGet = function(url, eventType){
			var request = http.get(url, function(res){
				response.log('beacon for tracking id [' + params.trackingId + '] type [' + eventType + '] url [' + url + '] sent with status: [' + res.statusCode + ']');
				res.on('data', function() { /* do nothing */ });
			});
			request.setTimeout( KalturaConfig.config.cloud.requestTimeout * 1000, function( ) {
				response.error('Failed to send beacon for tracking id [' + params.trackingId + '] type [' + eventType + '] url [' + url + '], timeout');
			});			
			request.on('error', function(e){
				response.error('Failed to send beacon for tracking id [' + params.trackingId + '] type [' + eventType + '] url [' + url + '], ' + e.message);
			});	
		};

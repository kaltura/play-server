...
var KalturaVastParser = require('../protocols/vast/KalturaVastParser');
...
KalturaAdIntegrationManager.prototype.getAdMediaFiles = function(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, encodingParamsArr, sessionId, uiConfConfig, downloadCallback, errorCallback) {
	...
	var doGetAdMediaFiles = function(evaluatedUrl){
		...
		KalturaVastParser.parse(evaluatedUrl, headers, vastRequestTimeOut*1000,
			function(adServerResponse) {
			...
			function (err)
			{
				var msg = 'Failed to get Ad server response [' + cuePoint.sourceUrl + '] cue-point [' + cuePoint.id + '] session [' + sessionId + '] partner [' + cuePoint.partnerId + '] entry [' + cuePoint.entryId + '] due to ' + err;
				response.error(msg);

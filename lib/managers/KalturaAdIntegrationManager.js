	...
	var sendBeaconForType = function(events, eventType){
		var httpGet = function(url, eventType){
			...
			request.setTimeout( KalturaConfig.config.cloud.requestTimeout * 1000, function( ) {
				response.error('Failed to send beacon for tracking id [' + params.trackingId + '] type [' + eventType + '] partner [' + params.partnerId + '] url [' + url + '], timeout');
			});			
			request.on('error', function(e){
				response.error('Failed to send beacon for tracking id [' + params.trackingId + '] type [' + eventType + '] partner [' + params.partnerId + '] url [' + url + '], ' + e.message);
			});	
		};

...
KalturaAdIntegrationManager.prototype.getAdMediaFiles = function(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, uiConfConfig, downloadCallback, errorCallback) {
	...
	var selectAdsByDuration = function(adServerResponse, duration){
		...
		if(selectedAdPod.length > 0){
			response.log('Selected Creatives ' + selectedAdPod.length + ' of total duration [' + adPodDuration + '] for partner [' + entry.partnerId +'] entry [' + entry.id + '] cue-point [' + cuePoint.id + '] session [' + sessionId +']');
			return selectedAdPod;
		}				
		else{
			response.log('No Ad Pod selected');
			if(selectedCreative){
				response.log('Selected Creatives 1 of total duration [' + selectedCreative.duration + '] for partner [' + entry.partnerId +'] entry [' + entry.id + '] cue-point [' + cuePoint.id + '] session [' + sessionId +']');
	...
	response.log('Parsing ads from [' + cuePoint.sourceUrl + '] cue-point [' + cuePoint.id + '] session [' + sessionId + '] partner [' + entry.partnerId + '] entry [' + entry.id + '] headers [' + JSON.stringify(headers) + ']');

...
KalturaAdIntegrationManager.prototype.getAdMediaFiles = function(request, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, uiConfConfig, downloadCallback, errorCallback) {
	...
	var setServerAdIdsInCache = function () {
		...
		KalturaLogger.log('Setting server ad ids in cache');
		...
		for(var serverAdIdKey in serverAdIdValues){
			KalturaLogger.log('Setting Server Ad ids for key [' + serverAdIdKey + '] values [' + JSON.stringify(serverAdIdValues[serverAdIdKey]) + ']');

...
KalturaAdIntegrationManager.prototype.getAdMediaFiles = function(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, encodingParamsArr, sessionId, uiConfConfig, sessionStartTime,  downloadCallback, errorCallback) {
	...
	var doGetAdMediaFiles = function(evaluatedUrl){
		...
		KalturaVastParser.parse(evaluatedUrl, headers, vastRequestTimeOut*1000, KalturaLogger, function(err, adServerResponse) {
			if (err) {
	...
	var headersCacheKey = KalturaCache.getKey(KalturaCache.HEADERS_PREFIX, [sessionId]);
	KalturaCache.get(
		headersCacheKey,
		function(value) {
			headers = value;
			if (value)
				...
				KalturaLogger.error('Failed to get headers for sessionId ' + sessionId + ' from getAdMediaFiles. Using default values');						
			KalturaCache.set(
				headersCacheKey,
				headers,
		...
		function(error) {
		    KalturaLogger.error('Failed to get headers for sessionId ' + sessionId + ' from getAdMediaFiles')
		    KalturaCache.set(
				headersCacheKey,
				headers,
				KalturaConfig.config.cache.sessionHeaders,

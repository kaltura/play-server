...
const KalturaTempFileManager = require('../utils/KalturaTempFileManager');
require('../utils/KalturaFileStatus');
...
class KalturaFetchManager extends kaltura.KalturaManager
{
			...
			function (mediaInfoForAd)
			{
				...
				KalturaFFMpegCmdGenerator.generateCommandLineFormat(flavorId, mediaInfoForAd.jsonInfo, null, This.apiConnector, partnerId).then(
					function (data)
					{
						...
						const commandLine = KalturaFFMpegCmdGenerator.fillCmdLineFormat(data, path, KalturaTempFileManager._getTempFilePath(outPath));
	...
	_transcodeAdAndSaveToDisk(path, flavorId, partnerId, callback)
	{
		...
		this._getAdTranscodingCommand(path, flavorId, partnerId,
			...
			{
				KalturaTempFileManager.getFileStatus(outPath).then( function (status){
					switch (status)
					{
						...
						case FILE_STATUS.PROCESSING:
							...
							setTimeout(This._transcodeAdAndSaveToDisk, 10*1000, path, flavorId, partnerId, callback);
							break;

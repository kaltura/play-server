...
class KalturaFetchManager extends kaltura.KalturaManager
{
	...
	_createFillerForFlavor(adsReadyCacheKey, flavorId, partnerId, fillerId)
	{
		...
		KalturaUtils.downloadHttpUrl(downloadUrl, { filePath: originalFillerLocalPath },
			function ()
			{
				...
				TranscodingHelper.transcodeExistingFileToDisk(flavorId, This.apiConnector, partnerId, originalFillerLocalPath, This.fillerPath, null, false).then(
					function(transcodedFileInfo)
					{
	...
	warmup(request, response, params)
	{
		...
		KalturaLogger.log(`Trying to add key ${params.fetchId}${FETCH_CACHE_SUFFIX}`);
		KalturaCache.add(`${params.fetchId}${FETCH_CACHE_SUFFIX}`, EMPTY_STRING, KalturaConfig.config.cache.fetchIdentifierLifetime,
			function(){
				KalturaLogger.log(`Handling fetch id ${params.fetchId}`);
				FetchIdentifier.getFetchId(params.fetchId, _handleFetchId);
			},function(err){
				KalturaLogger.log(`Fetch already started for fetch id : ${params.fetchId}`);
			});
	}

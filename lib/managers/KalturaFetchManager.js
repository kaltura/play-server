...
const KalturaUrlTokenMapper = require('../utils/KalturaUrlTokenMapper');
...
class KalturaFetchManager extends kaltura.KalturaManager
{
		...
		KalturaCache.append(adsReadyCacheKey, errorTag,
			...
			function (err)
			{
				...
				KalturaCache.del(adsReadyCacheKey,
					...
					function (err)
					{
						KalturaLogger.error(`CRITICAL! Failed to delete ads ready cache key ${adsReadyCacheKey} after errors occurred, due to ${err}`);
					}
				);
			}
		);
	 ...
	 * @private
	 */
	_createFillerForFlavor(adsReadyCacheKey, flavorId, partnerId, fillerId)
	{
		const This = this;
		...
		KalturaUtils.downloadHttpUrl(downloadUrl, { filePath: originalFillerLocalPath },
			function ()
			{
				let size = KalturaUtils.getFileSizeInBytes(originalFillerLocalPath);
				if (!size)
					rejectCallback(`Fail to download filler from ${downloadUrl} to path ${originalFillerLocalPath} with size of ${size}`);
	...
	_createFillers(adsReadyCacheKey, flavorList, partnerId, fillerId)
	{
		...
		for (let flavorIdx = 0; flavorIdx < flavorList.length; flavorIdx++)
		{
			...
			if (fillerId !== BLACK_FILLER)
				This._createFillerForFlavor(adsReadyCacheKey, flavorId, partnerId, fillerId);
							...
							function (adFileId)
							{
								...
								KalturaUtils.downloadHttpUrl(adFileUrl, { filePath: path },
									function (filePath)
									{
										...
										let size = KalturaUtils.getFileSizeInBytes(filePath);
										if (!size)
											reject(`Fail to download ad from ${adFileUrl} to path ${filePath} with size of ${size}`);
										This._transcodeAdAndSaveToDisk(filePath, flavorId, partnerId,
											function (outPath, adDurationInSeconds)
											{
												...
												This._updateCacheWithTranscodedAdData(adsReadyCacheKey, flavorId, adIdx, Math.floor(adDurationInSeconds * 1000), outPath, trackingList);
												resolve();
											}
										);
									},
	...
	warmup(request, response, params)
	{
		...
		function _handleFetchId(fetchId)
		{
			...
			// execution
			KalturaUrlTokenMapper.mapFixedTokensVod(request, cuePointId, vastUrl, partnerId, entryId, function(vastUrl){
				evaluatedUrl = KalturaUrlTokenMapper.mapDynamicTokens(request, vastUrl, playerConfig);
				This._getDurationFilteredVastAds(evaluatedUrl, request.headers, cuePointDuration).
			...
			function (mediaInfoForAd)
			{
				...
				KalturaFFMpegCmdGenerator.generateCommandLineFormat(flavorId, mediaInfoForAd.jsonInfo, null, This.apiConnector, partnerId).then(
					function (data)
					{
						...
						const commandLine = KalturaFFMpegCmdGenerator.fillCmdLineFormat(data, path, tmpPath);
	...
	_transcodeAdAndSaveToDisk(path, flavorId, partnerId, callback)
	{
		...
		this._getAdTranscodingCommand(path, flavorId, partnerId,
			...
			{
				KalturaTempFileHandler.getFileStatus(outPath).then( function (status){
					switch (status)
					{
						...
						case FILE_STATUS.PROCESSING:
							KalturaLogger.log(`File is currently transcoding on local disk ${outPath} - will retry in 10 seconds`);
							setTimeout(function(){This._transcodeAdAndSaveToDisk(path, flavorId, partnerId, callback)}, 10*1000);
							break;

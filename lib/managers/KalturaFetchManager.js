...
class KalturaFetchManager extends kaltura.KalturaManager
{
	...
	static generateAdsReadyCacheKey(cuePointId, flavorId, sessionId)
	{
		return KalturaCache.getKey(KalturaCache.VOD_SESSION_ADS_READY_KEY_PREFIX, [cuePointId, flavorId, sessionId]);
	}
		...
		KalturaCache.add(adKey, adCacheData, KalturaConfig.config.cache.adMedia,
			function ()
			{
				...
				KalturaCache.append(adBreakCacheKey, `#${adKey}`,
					...
					function ()
					{
						KalturaLogger.error(`Failed to append adKey due to cache failure to append: ${adKey} to key: ${adBreakCacheKey}`);
					}
				);
			},
			function (error)

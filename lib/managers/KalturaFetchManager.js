	...
	_createFillerForFlavor(adsReadyCacheKey, flavorId, partnerId, fillerId)
	{
		...
		KalturaUtils.downloadHttpUrl(downloadUrl, { filePath: originalFillerLocalPath },
			function ()
			{
				...
				TranscodingHelper.transcodeExistingFileToDisk(flavorId, This.apiConnector, partnerId, originalFillerLocalPath, This.fillerPath, null, true).then(
					function(transcodedFileInfo)
					{
							...
							function (adFileId)
							{
								...
								KalturaUtils.downloadHttpUrl(adFileUrl, { filePath: path },
									function (filePath)
									{
										...
										TranscodingHelper.transcodeExistingFileToDisk(flavorId, this.apiConnector, partnerId, filePath, filePath, null, false).then(
											function (transcodedFileInfo)
											{
											...
											function (err)
											{
												if(err == 'File is currently transcoding on local disk')
													resolve();//todo?? - maybe file is processing
												else

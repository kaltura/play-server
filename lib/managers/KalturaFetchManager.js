...
class KalturaFetchManager extends kaltura.KalturaManager
{
	...
	warmup(request, response, params)
	{
		...
		FetchIdentifier.getFetchId(params.fetchId,
			_handleFetchId,
			(err) => KalturaLogger.info(`Fail to load Fetch ID from cache due to - ${util.inspect(err)}`));
	}
	...
	initBlackFillers(request, response, params)
	{
		...
		Promise.all(blackFillers).then(function(){
			KalturaLogger.log(`Transcoded all black fillers`);
			CacheLock.lock(entryId,
				() => { LayoutHelper.updateEntryVersion(entryId,() => { KalturaLogger.log(`updated entry ${entryId} version in cache}`); CacheLock.unlock(entryId); },
					(err) => { KalturaLogger.log(`Could not update entry ${entryId} version, due to : ${util.inspect(err)}`);});
				}, ()=> {KalturaLogger.log(`Could not lock entry ${entryKey} ,this probably means someone already locked the entry`)});

		}, function (err){
			KalturaLogger.log(`Failed to transcode black fillers , due to ${util.inspect(err)}`);
		});
	}

...
class KalturaFetchManager extends kaltura.KalturaManager
{
	...
	_insertAdDataToCacheKey(adsReadyCacheKey, ads, flavorIdList, partnerId, cuePointOverlay = null, cuePointId = null)
	{
		for (let adIdx = 0; adIdx < ads.length; adIdx++)
		{
			let itemNumber = adIdx + 1;
			KalturaLogger.log(`Handling ad number ${itemNumber} out of ${ads.length}`);
			this._handleAd(adsReadyCacheKey, ads[adIdx], adIdx, flavorIdList, partnerId, cuePointOverlay, cuePointId);
		}
	}

	_handleAd(adsReadyCacheKey, ad, adIdx, flavorIdList, partnerId, cuePointOverlay = null, cuePointId = null)
	{
		const This = this;
		const trackingList = this._vastTrackingInfoToCacheTrackingInfo(ad);
		for (let i = 0; i < flavorIdList.length; i++)
		{
			this._handleAdForFlavor(flavorIdList[i], adsReadyCacheKey, ad, adIdx, partnerId, trackingList, cuePointOverlay, cuePointId).
				then(
				function ()
				{
	 ...
	 * @private
	 */
	_handleAdForFlavor(flavorId, adsReadyCacheKey, ad, adIdx, partnerId, trackingList, cuePointOverlay = null, cuePointId = null)
	{
		const This = this;
		return new Promise(
			function (resolve, reject)
							...
							function (adFileId)
							{
								...
								KalturaUtils.downloadHttpUrl(adFileUrl, { filePath: path },
									function (filePath)
									{
										...
										TranscodingHelper.transcodeExistingFileToDisk(flavorId, This.apiConnector, partnerId, filePath, filePath, null, true, cuePointOverlay, cuePointId).then(
											function (transcodedFileInfo)
											{
	...
	warmup(request, response, params)
	{
		...
		function _handleFetchId(fetchId)
		{
			...
			const cuePointOverlay = fetchId.cuePointOverlay;
			...
			function handleDurationFilteredAds(ads)
			{
				...
				createInitialCacheKeyForAdBreak(ads.length,
					...
					{
						This._createFillers(adsReadyCacheKey, flavorList, partnerId, fillerId);
						This._insertAdDataToCacheKey(adsReadyCacheKey, ads, flavorList, partnerId, cuePointOverlay, cuePointId);
					},
					printErrorLog);
			}
			// execution
			KalturaUrlTokenMapper.mapTokensVod(request, cuePointId, vastUrl, partnerId, entryId,
				(vastUrl) => This._getDurationFilteredVastAds(vastUrl, request.headers, cuePointDuration).

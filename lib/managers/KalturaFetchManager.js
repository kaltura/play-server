...
class KalturaFetchManager extends kaltura.KalturaManager
{
								...
								KalturaUtils.downloadHttpUrl(adFileUrl, { filePath: path },
									function (filePath)
									{
										...
										This._transcodeAdAndSaveToDisk(filePath, flavorId, partnerId,
											...
											function (err)
											{
												reject(adsReadyCacheKey, `Files are being processed , ${err}`);
											}
										);
									},
									function (err)
	...
	_transcodeAdAndSaveToDisk(path, flavorId, partnerId, callback, errorCallback)
	{
		const This = this;
				...
				KalturaTempFileHandler.getFileStatus(outPath).then( function (status){
					switch (status)
					{
						...
						case FILE_STATUS.PROCESSING:
							return errorCallback(`File is currently transcoding on local disk - skipping`);
						case FILE_STATUS.DOESNT_EXIST:
							...
							break;
						default:
							errorCallback(`Got invalid file status ${status} - please open a bug`);
							break;
					}
				}, function (err) {

...
class KalturaFetchManager extends kaltura.KalturaManager
{
	...
	warmup(request, response, params)
	{
       ...
       // FetchIdentifier.getFetchId(params.fetchId, _handleFetchId);
		KalturaLogger.log('Trying to add key ' + params.fetchId + 'x');
		KalturaCache.add(params.fetchId + 'x', '{}', 60, function(){
			KalturaLogger.log('Handling fetch id' + params.fetchId);
			FetchIdentifier.getFetchId(params.fetchId, _handleFetchId);},
			function(err){KalturaLogger.log('Fetch already started for fetch id : ' + params.fetchId);});
	}

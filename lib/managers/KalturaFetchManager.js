...
/* global KalturaLogger,KalturaCache,KalturaUtils,KalturaConfig,BLACK_FILLER,FILLER,TEN_RADIX,FLAVORS_SIGN_SEPARATOR,ERROR_FILE_IS_TRANSCODING */
...
class KalturaFetchManager extends kaltura.KalturaManager
{
	constructor()
	{
		...
		this.mediaInfo = new KalturaMediaInfo();
	}
		...
		KalturaCache.append(adsReadyCacheKey, errorTag,
			...
			function (err)
			{
				...
				KalturaCache.del(adsReadyCacheKey,
					function ()
					{
						KalturaLogger.debug(`Managed to delete key ${adsReadyCacheKey}`);
					},
					function (err)
					{
						KalturaLogger.error(`CRITICAL! Failed to delete ads ready cache key ${adsReadyCacheKey} after errors occurred, due to ${util.inspect(err)}`);
					}
				);
			}
		);
	...
	_createCustomFillerForFlavor(adsReadyCacheKey, flavorId, partnerId, fillerId)
	{
		...
		APIRequestHelper.getFlavorPath(fillerId, partnerId, null, null).then(
			function (originalFillerLocalPath)
			{
			...
			function (err)
			{
				This._failedToTranscodeFiller(adsReadyCacheKey, `Failed to download filler id given ${fillerId} due to ${util.inspect(err)}`);
			}
		);
	}

	_insertAdDataToCacheKey(adsReadyCacheKey, adsAndCreative, flavorIdList, partnerId)
	{
		for (let adIdx = 0; adIdx < adsAndCreative.length; adIdx++)
		{
			const itemNumber = adIdx + 1;
			KalturaLogger.log(`Handling ad number ${itemNumber} out of ${adsAndCreative.length}`);
			const adAndCreative = adsAndCreative[adIdx];
			this._handleAd(adsReadyCacheKey, adAndCreative.ad, adAndCreative.creative, adIdx, flavorIdList, partnerId);
		}
	}

	_handleAd(adsReadyCacheKey, ad, creative, adIdx, flavorIdList, partnerId)
	{
		const This = this;
		const trackingList = this._vastTrackingInfoToCacheTrackingInfo(ad, creative);
	 ...
	 * @private
	 */
	_getDurationFilteredVastAds(vastUrl, headers, cuePointDuration, partnerId, entryId, cuePointId, sessionId)
	{
		return new Promise(
			function (resolve, reject)
			{
				...
				KalturaVastParser.parse(vastUrl, headers, vastTimeout,
					function (vastResponse)
					{
						...
						const filteredAdsAndSelectedCreative = VastDurationFilter.filter(vastResponse, cuePointDuration, KalturaConfig.config.fetch.durationCoefficient, partnerId, entryId, cuePointId, sessionId);
						resolve(filteredAdsAndSelectedCreative);
					},
					function (err)
					{
						reject(`Failed to get vast response from url:${vastUrl} with headers: ${JSON.stringify(headers)}, and timeout ${vastTimeout} due to ${err}`);
					}
				);
			}
		);
										...
										TranscodingHelper.transcodeExistingFileToDisk(flavorId, This.apiConnector, partnerId, filePath, filePath, null, true).then(
											...
											function (err)
											{
												if (err.indexOf(ERROR_FILE_IS_TRANSCODING) == 0)
												{
													...
													reject({ adsReadyCacheKey, err: `Failed to transcode ad from path ${filePath} due to ${util.inspect(err)}` });
											}
										);
									},
									function (err)
									{
										reject({ adsReadyCacheKey, err: `Failed to download ad from url ${adFileUrl} due to ${util.inspect(err)}` });
									}
								);
							},
							function (err)
							{
								reject({ adsReadyCacheKey, err: `Failed to generate id for ad with the following URL: ${adFileUrl} due to ${util.inspect(err)}` });
							}
						);
					},
					function (err)
					{
						reject({ adsReadyCacheKey, err: `Failed to get flavor attributed for ${flavorId} , due to ${util.inspect(err)}` });
					}
				);
			}
		);
	...
	warmup(request, response, params)
	{
		...
		function _handleFetchId(fetchId)
		{
			...
			{
				const adsReadyCacheKey = AdBreakKeyHelper.getReadyAdsCacheKey(cuePointId, sessionId);
			...
			function handleDurationFilteredAds(adsAndCreative)
			{
				KalturaLogger.log(`Got ${adsAndCreative.length} ads filtered by duration`);
				createInitialCacheKeyForAdBreak(adsAndCreative.length,
					function (adsReadyCacheKey)
					{
						This._createFillers(adsReadyCacheKey, flavorList, partnerId, fillerId);
						This._insertAdDataToCacheKey(adsReadyCacheKey, adsAndCreative, flavorList, partnerId);
					},
					printErrorLog);
			}
			// execution
			KalturaUrlTokenMapper.mapTokensVod(request, cuePointId, vastUrl, partnerId, entryId,
				(vastUrl) => This._getDurationFilteredVastAds(vastUrl, request.headers, cuePointDuration, partnerId, entryId, cuePointId, sessionId).
					then(handleDurationFilteredAds, createBlackFillersAdBreak)
				, This.apiConnector);
		}
		FetchIdentifier.getFetchId(params.fetchId,
			_handleFetchId,
			(err) => KalturaLogger.error(`Fail to load Fetch ID from cache due to - ${util.inspect(err)}`));
	}
	 ...
	 * @private
	 */
	_vastTrackingInfoToCacheTrackingInfo(ad, creative)
	{
		const trackingInformation = VastTrackingParser.getTrackingInformation(ad, creative);
		const result = [];
		for (let i = 0; i < trackingInformation.length; i++)
		{
			const type = trackingInformation[i].key;
			const value = trackingInformation[i].value;
			if (type !== 'skipOffset') // skip offset is numeric
				result.push(new BeaconCacheData(type, value.trim()));
			else
				result.push(new BeaconCacheData(type, value));
		}
		return result;
	}
	...
	_updateCacheOnAdOrFiller(adBreakCacheKey, adCacheData)
	{
		...
		KalturaLogger.debug(`Adding (type:${adCacheData.type}) adkey:${adKey}`);
		KalturaCache.add(adKey, adCacheData, KalturaConfig.config.cache.adMedia,
			...
			function (err)
			{
				KalturaLogger.error(`Failed to add key to cache for new ad - ${adKey}, due to ${util.inspect(err)}`);
			}
		);
	}
	...
	{
		const paramsDuplicate = this.parsePlayServerParams(response, params, ['flavorIds', 'partnerId', 'entryId']);
		response.dir(paramsDuplicate);
		this.okResponse(response, 'OK', 'text/plain');
		if (!paramsDuplicate)
			return;
		const flavorIds = paramsDuplicate.flavorIds.split(FLAVORS_SIGN_SEPARATOR);
		const entryId = paramsDuplicate.entryId;
		const blackFillers = [];
		for (const flavorId of flavorIds)
		{
			blackFillers.push(TranscodingHelper.transcodeBlackFillerToDisk(
				flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs, this.apiConnector, params.partnerId, PathsGenerator.getBlackFillerLocalPrefixPath()));
		}
		Promise.all(blackFillers).then(
			function ()
			{
				KalturaLogger.log('Transcoded all black fillers');
				CacheLock.lock(entryId,
					function ()
					{
						LayoutHelper.updateEntryVersion(entryId,
							function ()
							{
								KalturaLogger.debug(`updated entry ${entryId} version in cache}`);
								CacheLock.unlock(entryId);
							},
							function (err)
							{
								KalturaLogger.log(`Could not update entry ${entryId} version, due to : ${util.inspect(err)}`);
								CacheLock.unlock(entryId);
							}
						);
					},
					() => { KalturaLogger.log(`Could not lock entry ${entryId} ,this probably means someone already locked the entry`) }
				);
			},
			function (error)
			{
				KalturaLogger.error(`Failed to transcode black fillers , due to ${util.inspect(error)}`);
			}
		);
	}

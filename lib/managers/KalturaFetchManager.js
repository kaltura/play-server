...
class KalturaFetchManager extends kaltura.KalturaManager
{
	constructor()
	{
		...
		this.fillerPath = KalturaConfig.config.cloud.sharedBasePath + '/filler/';
	}
	...
	_createFillerForFlavor(flavorId, sessionId, partnerId, cuePointId, fillerId)
	{
		...
		return new Promise(
			function (resolve, reject)
			{
				...
				KalturaCache.add(adsReadyCacheKey, 'Ads : #', KalturaConfig.config.cache.adMedia,
					function ()
					{
						if (fillerId === BLACK_FILLER)
						{
							KalturaLogger.log(`Constructing black filler for flavor if ${flavorId}`);
							TranscodingHelper.transcodeBlackFillerToDisk(flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs, This.apiConnector, partnerId, This.fillerPath).bind(This).then(
								function (blackFillerLocalPath)
								{
									KalturaLogger.log(`Managed to transcode and save file ${blackFillerLocalPath}`);
									This._updateCacheWithTranscodedFillerData(adsReadyCacheKey, KalturaConfig.config.layout.fillerDefaultDurationSecs * 1000, blackFillerLocalPath);
									resolve({ adsReadyCacheKey, flavorId });
								},
								function (err)
								{
									const message = `Failed to transcode black filler file to path due to error: ${err}`;
						...
						else
						{
							...
							TranscodingHelper.transcodeExistingFileToDisk(flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs, This.apiConnector, partnerId, originalFillerLocalPath, This.fillerPath).bind(This).then(
								function (fillerLocalPath)
								{
									KalturaLogger.log(`Managed to transcode and save file ${fillerLocalPath}`);
					...
					function ()
					{
						reject(`Already handling adsReadyCacheKey ${adsReadyCacheKey}`);
					}
				);
			}
		);
	...
	_createFillers(flavorList, sessionId, partnerId, cuePointId, fillerId)
	{
		...
		return new Promise(
			function (resolve, reject)
			{
				...
				Promise.all(promises).bind(This).then(
	...
	warmup(request, response, params)
	{
		...
		function handleAds(adsReadyCacheKeyList, ad)
		{
			...
			for (let adReadyCacheIdx = 0; adReadyCacheIdx < adsReadyCacheKeyList.length; adReadyCacheIdx++)
			{
				...
				This._handleAdForFlavor(adReadyPair.flavorId, adReadyPair.adsReadyCacheKey, ad, partnerId, trackingList).bind(This).
					then(
					function ()
					{
						KalturLogger.log(`Successfully cacluated ads for key ${adReadyPair.adsReadyCacheKey}`);
					},
		...
		this._getDurationFilteredVastAds(vastUrl, request.headers, cuePointDuration).then(
			function (ads)
			{
				for (let adIdx = 0; adIdx < ads.length; adIdx++)
					This._createFillers(flavorList, params.sessionId, partnerId, cuePointId, fillerId).bind(This).then(
						function(adsReadyCacheKeyList)
						{
							handleAds(adsReadyCacheKeyList, ads[adIdx]);
						}, printErrorLog);
			},
			printErrorLog
		);
		...
		//			{
		//				KalturaLogger.log(`Constructing black filler for flavor if ${flavorId}`);
		//				TranscodingHelper.transcodeBlackFillerToDisk(flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs, This.apiConnector, partnerId, This.fillerPath).then(
		//					function (blackFillerLocalPath)
		//					{
		//						KalturaLogger.log(`Managed to transcode and save file ${blackFillerLocalPath}`);
		//						This._updateCacheWithTranscodedFillerData(adsReadyCacheKey, KalturaConfig.config.layout.fillerDefaultDurationSecs * 1000, blackFillerLocalPath);
		...
		//					function (err)
		//					{
		//						return This._markAdsReadyAsError(adsReadyCacheKey, `Failed to transcode black filler file to path ${blackFillerLocalPath}  due to error: ${err}`);
		//					}
		//				);
		//			}
		//			else
		//			{
		//				const originalFillerLocalPath = PathsGenerator.getOriginFillerLocalPath(fillerId);
		//				TranscodingHelper.transcodeExistingFileToDisk(flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs, This.apiConnector, partnerId, originalFillerLocalPath, This.fillerPath).then(
		//					function (fillerLocalPath)
		//					{
		//						KalturaLogger.log(`Managed to transcode and save file ${fillerLocalPath}`);
		//						This._updateCacheWithTranscodedFillerData(adsReadyCacheKey, KalturaConfig.config.layout.fillerDefaultDurationSecs * 1000, fillerLocalPath);
		...
		//					function (err)
		//					{
		//						return This._markAdsReadyAsError(adsReadyCacheKey, `Failed to transcode black filler file to path ${fillerLocalPath}  due to error: ${err}`);
		//					}
		//				);
		//			}
		//		},
		...
		//		if (!vastResponse)
         //       {
		//			This.errorResponse(response, 412, `Failed to get vast response from url:${vastUrl} with headers: ${JSON.stringify(request.headers)}, and timeout ${vastTimeout}`);
		//			return;
         //       }
		//		KalturaLogger.log(`Got vast response for vast url:${vastUrl}, response:${JSON.stringify(vastResponse)}`);
		//		const filteredAds = VastDurationFilter.filter(vastResponse, cuePointDuration, KalturaConfig.config.fetch.durationCoefficient);
		...
		//									function(err)
		//									{
		//										return This._markAdsReadyAsError(adsReadyCacheKey, `Failed to download ad from url ${adFileUrl} due to ${err}`);
		//									}
		//								);
		//							},
		//							function (err)
		//							{
		//								return This._markAdsReadyAsError(adsReadyCacheKey, `Failed to generate id for ad with the following URL: ${adFileUrl} due to ${err}`);
		//							}
		//						);
		//					},
		//					function(err)
		//					{
		//						return This._markAdsReadyAsError(adsReadyCacheKey, `Failed to get flavor attributed for ${flavorId} , due to ${err}`);
		//					}
		//				);
		//			}
		//		}
			...
			function (mediaInfoForAd)
			{
				...
				KalturaFFMpegCmdGenerator.generateCommandLineFormat(flavorId, mediaInfoForAd.jsonInfo, null, This.apiConnector, partnerId).then(
					function (data)
					{
						...
						const outPath = PathsGenerator.generateSpecificTranscodedPath(path, data);

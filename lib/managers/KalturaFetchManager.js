...
class KalturaFetchManager extends kaltura.KalturaManager
{
	...
	_createFillerForFlavor(flavorId, sessionId, partnerId, cuePointId, fillerId, numberOfAds)
	{
		...
		return new Promise(
			function (resolve, reject)
			{
				...
				const adsReadyDefaultCacheKey = KalturaCache.getKey(KalturaCache.VOD_SESSION_DEFAULT_ADS_READY_KEY_PREFIX, [cuePointId, flavorId, sessionId]);
	...
	warmup(request, response, params)
	{
		...
		function handleAd(adsReadyCacheKeyList, ad, adIdx)
		{
			...
			{
				const adReadyInfo = adsReadyCacheKeyList[adReadyCacheIdx];
				if (defaultsKey === null)

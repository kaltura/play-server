			...
			KalturaCache.add(adsReadyCacheKey, '#', KalturaConfig.config.cache.adMedia,
				function ()
				{
					if (fillerId === BLACK_FILLER)
					{
						...
						TranscodingHelper.transcodeBlackFillerToDisk(flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs/1000, This.apiConnector, partnerId, KalturaConfig.config.cloud.sharedBasePath).then(
							function(blackFillerLocalPath)
							{
								KalturaLogger.log(`Managed to transcode and save file ${blackFillerLocalPath}`);
								This._updateCacheWithTranscodedFillerData(adsReadyCacheKey, KalturaConfig.config.layout.fillerDefaultDurationSecs, blackFillerLocalPath);
							},
							function(err)
							{
								KalturaLogger.error(`Failed to transcode black filler file to path ${blackFillerLocalPath}  due to error: ${err}`);
							}
						);
					...
					else
					{
						...
						TranscodingHelper.transcodeExistingFileToDisk(flavorId, KalturaConfig.config.layout.fillerDefaultDurationSecs/1000, This.apiConnector, partnerId, originalFillerLocalPath, KalturaConfig.config.cloud.sharedBasePath).then(
							function(fillerLocalPath)
							{
								KalturaLogger.log(`Managed to transcode and save file ${fillerLocalPath}`);
								This._updateCacheWithTranscodedFillerData(adsReadyCacheKey, KalturaConfig.config.layout.fillerDefaultDurationSecs, fillerLocalPath);
							},
							function(err)
							{
								KalturaLogger.error(`Failed to transcode black filler file to path ${fillerLocalPath}  due to error: ${err}`);
							}
						);
					}
				},
					...
					function(err)
					{
						...
						else
						{
							engine.transcodeFile(commandLine).then(
								function (data){
									if(KalturaConfig.config.h264Verfication.enabled == 1)
										verifier.verifyAd(flavorId, outPath);

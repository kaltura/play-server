 ...
 * @param slateContent
 * @param fillerFilePath
 */
KalturaSegmentManager.prototype.execFiller = function(renditionId, fillerEncodingParams, fillerSourceFilePath, fillerOutputFilePath){	
	...
	fs.exists(fillerOutputFilePath, function(exists){
		if(exists){
			...
			This.saveFiller(renditionId, fillerOutputFilePath);
		}
		else{
 ...
 * @param fillerFilePath
 */
KalturaSegmentManager.prototype.saveFiller = function(renditionId, fillerOutputFilePath){	
	var ffprobeBin = KalturaConfig.config.bin.binDir + '/ffprobe';
	if(KalturaConfig.config.bin.ffprobePath){
		ffprobeBin = KalturaConfig.config.bin.ffprobePath;
	}
	...
	kaltura.tsPreparer.prepareTsFiles(ffprobeBin, [fillerOutputFilePath], function(err, data){
		...
		var fillerMediaKey = KalturaCache.getKey(KalturaCache.FILLER_MEDIA_KEY_PREFIX, [renditionId]);
...
KalturaSegmentManager.prototype.stitchFiller = function(request, response, params){
	...
	var This = this;
	var fillerEncodingParamsKey = KalturaCache.getKey(KalturaCache.FILLER_ENCODING_PARAMS_KEY_PREFIX, [params.renditionId]);
	KalturaCache.get(fillerEncodingParamsKey, function(fillerEncodingParams){
		...
		var fillerOutputPath = KalturaUtils.buildFilePath('filler',fileName);
		
		var fillerSourceFilePath = null;
		if(params.slateContent){
			fillerSourceFilePath = KalturaUtils.buildFilePath('filler', params.slateContent);
			//TODO build download URL
			KalturaUtils.downloadHttpUrl(params.slateContent, {filePath: fillerSourceFilePath}, function(fillerSourceFilePath){
				This.execFiller(params.renditionId, fillerEncodingParams, fillerSourceFilePath, fillerOutputPath);
			}, function(err){
				KalturaLogger.error('Failed to download filler [' + params.slateContent + ']');
				return;
			});
		}
		else{

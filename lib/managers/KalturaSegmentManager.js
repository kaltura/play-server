...
KalturaSegmentManager.prototype.stitchBlack = function(request, response, params){
	...
	var outputPath = KalturaConfig.config.cloud.sharedBasePath + '/tmp/black-' + params.renditionId;
	...
	var This = this;
	var blackEncodingParamsKey = KalturaCache.getKey(KalturaCache.BLACK_ENCODING_PARAMS_KEY_PREFIX, [params.renditionId]);
	KalturaCache.get(blackEncodingParamsKey, function(blackEncodingParams){
		...
		cmd.exec(function(){
			kaltura.tsPreparer.prepareTsFiles(ffprobePath, [outputPath], function(err, data){
				...
				var blackMediaKey = KalturaCache.getKey(KalturaCache.BLACK_MEDIA_KEY_PREFIX, [params.renditionId]);

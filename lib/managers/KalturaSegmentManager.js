...
KalturaSegmentManager.prototype.exec = function(segmentId, cutOffset, portion, inputFiles, callback, errorCallback){
	...
	kaltura.tsPreparer.cutTsFiles(ffmpegBin, ffprobeBin, cutOffset, leftPortion, inputFiles, function(err, data){
		...
		kaltura.tsPreparer.savePreparedTsToMemcache(KalturaCache.getBinServerSetByKey(segmentMediaKey), segmentMediaKey, data, KalturaSegmentManager.MAX_DVR_LENGTH, function(error){
			if(error){
 ...
 */
KalturaSegmentManager.prototype.execFiller = function(partnerId, renditionId, uiConfConfigId, slateContent, fillerEncodingParams, fillerSourceFilePath, fillerOutputFilePath){	
					...
					if(exists){
						...
						This.getFlavorUrl(partnerId, slateContent, function(fillerDownloadUrl){
							if (fillerDownloadUrl === null ){
								KalturaLogger.error('Failed to get filler [' + slateContent + '] download URL');
								return;
							}
...
KalturaSegmentManager.prototype.saveFiller = function(renditionId, uiConfConfigId, fillerOutputFilePath){	
	...
	kaltura.tsPreparer.prepareTsFiles(ffprobeBin, [fillerOutputFilePath], function(err, data){
		...
		kaltura.tsPreparer.savePreparedTsToMemcache(KalturaCache.getBinServerSetByKey(fillerMediaKey), fillerMediaKey, data, KalturaSegmentManager.MAX_DVR_LENGTH, function(error){
			if(error){
...
KalturaSegmentManager.prototype.stitchFiller = function(request, response, params){
	...
	KalturaCache.get(fillerEncodingParamsKey, function(fillerEncodingParams){
		...
		if(params.slateContent){
			...
			fillerSourceFilePath = KalturaConfig.config.cloud.sharedBasePath + '/filler/' + params.slateContent;
		}

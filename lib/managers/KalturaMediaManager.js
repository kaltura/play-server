	...
	KalturaCache.get(serverAdIdKey, function(serverAdIds){
		if(!serverAdIds){
			...
			for(var i = 0; i< serverAdIds.length; i++){
				...
				adsSequence.push({id:serverAdIds[i].id, startPos:startPos, endPos:endPos});
			}
			

			for(var j = startSequenceIndex; adsSequence[j] && (adsSequence[j].startPos <= outputEnd || !outputEnd); j++){
				currentAdsIdx.push(j);
			...
			KalturaAdIntegrationHandler.sendBeacon(trackingKey, currentAdsIdx, sequenceDuration, segmentIndex, outputStart, outputEnd);

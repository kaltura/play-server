		...
		if(!serverAdIds){
			...
			for(var i = 0; serverAdIds[i]; i++){
				if(sequenceDuration + serverAdIds[i].duration <= outputStart){
					...
					var startPos = 0;
					var maxEndPos = outputEnd - outputStart;
					var endPos = Math.min((serverAdIds[i].duration - (outputStart - sequenceDuration)), maxEndPos); 
					for(var j = i; serverAdIds[j] && endPos <= maxEndPos; j++){
						adSequence.push(i);
						serverAdId.push({id:serverAdIds[j].id, startPos:startPos, endPos:endPos});
						startPos = endPos;
						endPos += serverAdIds[i].duration; 
						endPos = Math.min(endPos, maxEndPos);						
					}					
					break;
				}
			}
			...
			KalturaAdIntegrationHandler.sendBeacon(trackingKey, adSequence, sequenceDuration, segmentIndex, outputStart, outputEnd);

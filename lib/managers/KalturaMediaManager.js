...
KalturaMediaManager.prototype.segment = function(request, response, params){
	...
	var serverAdIdKey = KalturaCache.getServerAdId(params.cuePointId, params.encodingId);
	
	KalturaCache.get(serverAdIdKey, function(serverAdId){
		...
		response.writeHead(302, {
				...
				'Cache-Control': KalturaConfig.config.stream.cdnCacheScope + ', max-age=' + KalturaConfig.config.stream.cdnMaxAge + ', max-stale=0'
		});
		response.end();		
	}, function(err){

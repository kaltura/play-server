...
KalturaMediaManager.MAX_DVR_LENGTH = 24 * 60 * 60;
...
KalturaMediaManager.prototype.get = function(request, response, params){
	...
	var slatePostSegmentId = KalturaCache.getPostSegmentId(params.cuePointId, params.serverAdId + '-' + segmentIndex);
	...
	var postSegmentStitchedKey = KalturaCache.getKey(KalturaCache.POST_SEGMENT_STITCHED_KEY_PREFIX, 
			[KalturaCache.getPostSegmentStitchedId(params.cuePointId, params.adsSequenceId, segmentIndex)]);
	var prevPostSegmentStitchedKey = KalturaCache.getKey(KalturaCache.POST_SEGMENT_STITCHED_KEY_PREFIX, 
														[KalturaCache.getPostSegmentStitchedId(params.cuePointId, params.adsSequenceId, segmentIndex-1)]);
	var prev2PostSegmentStitchedKey = KalturaCache.getKey(KalturaCache.POST_SEGMENT_STITCHED_KEY_PREFIX, 
														[KalturaCache.getPostSegmentStitchedId(params.cuePointId, params.adsSequenceId, segmentIndex-2)]);
	...
	KalturaCache.getMultiBinary(cacheKeys, function(data){
		...
		if(stitchPostSegment && !data[slatePostAdMetadataKey]){
			...
			This.stitchPostSegment(response, params.originalUrl, slatePostSegmentId, adsMetadata, outputStart, adStart, cuePointDuration, function(metadata){
				if(metadata){
					KalturaCache.set(postSegmentStitchedKey, true, KalturaMediaManager.MAX_DVR_LENGTH);
 ...
 * 
 */
KalturaMediaManager.prototype.stitchPostSegment = function(response, originalUrl, segmentId, adsMetadata, outputStart, adStart, cuePointDuration, callback){
	...
	KalturaUtils.downloadHttpUrl(originalUrl, {filePath: filePath}, function(filePath){
		...
		kaltura.tsPreparer.rightCutOnIFrame(ffprobeBin, cutOffset, maxCutOffset, [filePath], function(err, data){
			if(err){
			...
			kaltura.tsPreparer.savePreparedTsToMemcache(KalturaCache, segmentMediaKey, data, KalturaMediaManager.MAX_DVR_LENGTH, function(error){
				if(error){
	...
	KalturaCache.get(serverAdIdKey, function(serverAdIds){
		if (!serverAdIds){
			...
			This.decideCanPassthrough(response, params.uiConfConfigId, serverAdIds, cuePointDuration, adStart, outputStart, outputEnd, maxSegmentDuration, function(slateState){
				...
				params.adsSequenceId = KalturaCache.buildAdsSequenceId(serverAdIds);
 ...
 */
KalturaMediaManager.prototype.decideCanPlayAd = function(response, entryId, cuePointId, sessionId, renditionId, sessionStartTime, originDc, cuePointTriggeredAt, callback){
            ...
            if(entryRequired){
                ...
                KalturaCache.getMulti(mediaInfoKeys, function(mediaInfos) {
                    ...
                    for(var mediaInfoId in mediaInfos) {
                        ...
                        var renditionFromMediaInfo = KalturaCache.getRenditionIdFromMediaInfo(mediaInfoId);
                        allSessionServerAdIdsKeys.push(KalturaCache.getKey(KalturaCache.SERVER_AD_ID_KEY_PREFIX, [cuePointId, renditionFromMediaInfo, sessionId]));
                    }

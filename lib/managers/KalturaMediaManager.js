...
KalturaMediaManager.prototype.get = function(request, response, params){
	...
	response.dir(params);
	this.serve(request, response, params.serverAdId, params.cuePointId, params.renditionId, parseInt(params.segmentIndex), parseInt(params.outputStart), parseInt(params.outputEnd), params.originalUrl, params.uiConfConfigId);
};

KalturaMediaManager.prototype.serve = function(request, response, serverAdId, cuePointId, renditionId, segmentIndex, outputStart, outputEnd, originalUrl, uiConfConfigId){	
	var This = this;
	...
	var fillerMetadataKey = KalturaCache.getKey(KalturaCache.FILLER_MEDIA_KEY_PREFIX, [renditionId, uiConfConfigId], KalturaCache.METADATA_KEY_SUFFIX);
	...
	KalturaCache.getMultiBinary(cacheKeys, function(data){
		...
		if (!preAdMetadata){
			...
			if (!adsMetadata.length){
				...
				var fillerKey = KalturaCache.getKey(KalturaCache.FILLER_MEDIA_KEY_PREFIX, [renditionId, uiConfConfigId]);

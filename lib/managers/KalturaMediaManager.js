...
KalturaMediaManager.prototype.get = function(request, response, params){
	...
	var prevSlatePostSegmentId = KalturaCache.getPostSegmentId(params.cuePointId, params.serverAdId + '-' + (segmentIndex-1));
	var prev2SlatePostSegmentId = KalturaCache.getPostSegmentId(params.cuePointId, params.serverAdId + '-' + (segmentIndex-2));
	...
	var prev2SlatePostAdMetadataKey = KalturaCache.getKey(KalturaCache.SEGMENT_MEDIA_KEY_PREFIX, [prev2SlatePostSegmentId], KalturaCache.METADATA_KEY_SUFFIX);
	...
	if(stitchPostSegment){
		...
		cacheKeys.push(prev2SlatePostAdMetadataKey);
	}
	...
	KalturaCache.getMultiBinary(cacheKeys, function(data){
		...
		if(stitchPostSegment && (data[prevSlatePostAdMetadataKey] && data[prevSlatePostAdMetadataKey].length > 0 || 
								 data[prev2SlatePostAdMetadataKey] && data[prev2SlatePostAdMetadataKey].length > 0)){

		...
		if(entryRequired){
			...
			KalturaCache.getMulti(allServerAdIdsKeys, function(allServerAdIds){
				if(!allServerAdIds || Object.keys(allServerAdIds).length == 0){
					callback('no', '1:Server Ad Ids not found in cache, redirecting to original ts: requested after ' + preparationTime + ' seconds');
					return;
				}				
				else{
					var metadataKeys = [];
					...
					for(var serverAdIdKey in allServerAdIds){
						if(!allServerAdIds[serverAdIdKey]){
							callback('no', '2:Server Ad ids missing for key ' + serverAdIdKey + ', redirecting to original ts: requested after ' + preparationTime + ' seconds');
							return;
						}
						else{

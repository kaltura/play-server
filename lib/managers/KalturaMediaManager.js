...
KalturaMediaManager.prototype.stitchSegment = function(request, response, params, serverAdIdKey){
	...
	KalturaCache.get(serverAdIdKey, function(serverAdIds){
		if (!serverAdIds){
			response.log('Alert: serverAdIds not found in cache, redirecting to original ts');
	...
	KalturaCache.get(entryRequiredKey, function(entryRequired){
		...
		if(entryRequired){
			...
			KalturaCache.getMulti(allServerAdIdsKeys, function(allServerAdIds){
				if(!allServerAdIds){
					callback(false, '1:Server Ad Ids not found in cache, redirecting to original ts');
					return;
				}
				else{
					var metadataKeys = [];
					...
					for(var serverAdIdKey in allServerAdIds){
						if(!allServerAdIds[serverAdIdKey]){
							callback(false, '2:Server Ad ids missing for key ' + serverAdIdKey + ', redirecting to original ts');
							return;
						}
						else{
					...
					KalturaCache.getMulti(metadataKeys, function(data){
						for(var i=0; i<metadataKeys.length; i++){
							if(!(data[metadataKeys[i]])){
								callback(false, '3:Metadata missing for ' + metadataKeys[i] + ', redirecting to original ts');
								return;
							}
						}
						callback(true, null);
						...
						callback(false, '5:Error getting metadata status from cache, redirecting to original ts');
						return;
					});
				}

			}, function (err){
				callback(false, '6:Server Ad Ids not found in cache, redirecting to original ts');	
				return;
			});
		}			
	}, function(err){
		callback(false, '7:entryRequired not found in cache, redirecting to original ts');			
		return;
	});
};
...
KalturaMediaManager.prototype.segment = function(request, response, params){
	...
	if(segmentIndex == 0){
		This.decideCanPlayAd(response, params.entryId, params.cuePointId, params.sessionId, params.renditionId, function(shouldStitch, redirectError){
			...
			response.log('canPlayAd set to [' + shouldStitch + ']');
			if(shouldStitch){
		...
		KalturaCache.get(canPlayAdKey, function(canPlayAd){
			...
			response.log('canPlayAd is not set, redirecting to original ts');
			response.writeHead(302, {'Location' : params.originalUrl});
			response.end();	
		});
	}

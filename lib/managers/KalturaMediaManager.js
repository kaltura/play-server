...
KalturaMediaManager.prototype.serve = function(request, response, serverAdId, cuePointId, renditionId, segmentIndex, outputStart, outputEnd, originalUrl){	
	...
	var fillerMetadataKey = KalturaCache.getKey(KalturaCache.FILLER_MEDIA_KEY_PREFIX, [renditionId], KalturaCache.METADATA_KEY_SUFFIX);
	...
	KalturaCache.getMultiBinary(cacheKeys, function(data){
		...
		if (!preAdMetadata){
			...
			if (!adsMetadata.length){
				...
				var fillerKey = KalturaCache.getKey(KalturaCache.FILLER_MEDIA_KEY_PREFIX, [renditionId]);

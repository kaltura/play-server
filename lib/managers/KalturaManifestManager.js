 ...
 */
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, partnerId, entryId, uiConfId, sessionId, callback){
	var This = this;
	var encodingIds = [];
	var mediaInfos = null;
	var entry = null;
	...
	KalturaCache.getMulti([cuePointsKey, elapsedTimeKey, entryRequiredKey, playerConfigKey, uiConfConfigKey], function(data){
		...
		if(!data[uiConfConfigKey])
			KalturaLogger.log('ui conf config not loaded for session ' + sessionId + ' ui conf id ' + uiConfId);
...
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, partnerId, entryId, uiConfId, renditionId, masterUrl, sessionId){
	...
	var handleManifestContent = function(){
		...
		KalturaCache.get(manifestContentKey, function(manifestContent) {
			if(manifestContent){
				...
				if(trackCuePoints){
					This.stitchCuePoints(request, response, partnerId, entryId, uiConfId, sessionId, function(){
	...
	KalturaCache.get(uiConfConfigKey, function(uiConfConfig) {
		if(!uiConfConfig){
			This.getAndStoreUiConfConfig(uiConfId, entryId, partnerId, function(uiConfConfig) {
				if(uiConfConfig){
	...
	var playerConfigKey = KalturaCache.getPlayerConfig(sessionId);
	KalturaCache.get(playerConfigKey, function(playConfig) {
		if(playConfig){
			KalturaCache.touch(playerConfigKey, KalturaConfig.config.cache.playerConfig);
		}
		else{
			KalturaLogger.log('Failed to load player config for session ' + sessionId); 
		}
	}, function(err) {

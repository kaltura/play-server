...
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, entryId, sessionId, callback){
	...
	var stitchAd = function(cuePoint, cachedUrl) {
		
		KalturaAdIntegrationHandler.getAdMediaFiles(request, response, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, function(adFileId, adFileInfo){
		...
		KalturaCache.getMulti(encodingIdsKeys, function(mediaInfoData){
			mediaInfos = mediaInfoData;
	   		...
	   		for(i=0; i < cuePointsForStitching.length; i++){
	   			var cuePointUrlKey = KalturaCache.getCuePointUrl(cuePointsForStitching[i]);
	   			KalturaCache.get(cuePointUrlKey, function(cachedUrl){
	   				if(cachedUrl){
	   					response.log('Cue point url found in cache: [' + cachedUrl + ']');
	   					stitchAd(cuePointsForStitching[i], cachedUrl);	
	   				}
	   				else{
	   					...
	   					if(!entry){
	   						getEntryAndMetadata(function(entryObj, metadataObj){
	   							...
	   							stitchAd(cuePointsForStitching[i], null);
	   						});
	   					}	   						
	   				}
	   			}, function (err) {
   					...
   					if(!entry){
   						this.getEntryAndMetadata(function(entryObj, metadataObj){
   							entry = entryObj;
   							metadata = metadataObj;
   							stitchAd(cuePointsForStitching[i], null);
   						});

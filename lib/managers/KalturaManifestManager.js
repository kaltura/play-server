...
KalturaManifestManager.prototype.startWatcherExclusive = function(partnerId, entryId, manifestUrl, renditionsWatchParams) {
	...
	for(var i = 0; i < renditionsWatchParams.length; i++){
		entryRequired = KalturaCache.buildEntryRequiredValue(entryRequired, renditionsWatchParams[i].url);
	}	
	KalturaCache.set(entryRequiredKey, entryRequired, KalturaConfig.config.cache.entryHandled, function(){
		KalturaLogger.log('Entry [' + entryId + '] url [' + manifestUrl + '] setting entry required');
	}, function (err) {
		KalturaLogger.error('Entry [' + entryId + '] url [' + manifestUrl + '] failed to set entryRequired');
	});

...
KalturaManifestManager.prototype.startStreamWatcher = function(partnerId, entryId, manifestUrl, renditionWatchParams) {
	...
	var renditionManifestHandledKey = KalturaCache.getKey(KalturaCache.RENDITION_MANIFEST_HANDLED_KEY_PREFIX, [KalturaCache.getManifestId(renditionWatchParams.url)]);
...
KalturaManifestManager.prototype.stitchMasterM3U8 = function(partnerId, entryId, uiConfId, manifestUrl, trackCuePoints, manifestContent) {
	...
	if(trackCuePoints){
		...
		var manifestContentKey = KalturaCache.getKey(KalturaCache.MANIFEST_CONTENT_KEY_PREFIX, [manifestId]);
		var cache = {
			body: result,
...
KalturaManifestManager.prototype.master = function(request, response, params){
	...
	This.isPermissionAllowedForPartner(params.partnerId, 'FEATURE_PLAY_SERVER', function(isAllowed) {
		if(isAllowed && params.uiConfId){
				var uiConfConfigKey = KalturaCache.getKey(KalturaCache.UI_CONF_CONFIG_KEY_PREFIX, [params.uiConfId]);
				...
				KalturaCache.get(uiConfConfigKey, function(uiConfConfig) {
					if(!uiConfConfig){
						...
						KalturaCache.touch(uiConfConfigKey, KalturaConfig.config.cache.masterManifest);
						doFetchMaster(uiConfConfig.trackCuePoints);
					}
				});
		}
 ...
 * @param entryId
 */
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, partnerId, entryId, uiConfId, renditionId, masterUrl, sessionId, playerConfig, sessionStartTime, originDc){
	var This = this;
	var manifestContentKey = KalturaCache.getKey(KalturaCache.MANIFEST_CONTENT_KEY_PREFIX, [renditionId]);
...
KalturaManifestManager.prototype.rendition = function(request, response, params){
	...
	this.getRenditionFromCache(request, response, params.partnerId, params.entryId, params.uiConfId, params.renditionId, masterUrl, params.sessionId, params.playerConfig, params.sessionStartTime, params.originDc);

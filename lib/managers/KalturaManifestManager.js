...
require('../lib/utils/KalturaUiConfParser');
...
KalturaManifestManager.prototype.master = function(request, response, params){
	...
	if(parmas.ui_conf){		
		if(!params.playerConfig)
			params.playerConfig = { "uiConfId": parmas.ui_conf };
		else
			params.playerConfig.uiConfId = parmas.ui_conf;
	}	
...
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, entryId, sessionId, callback){
	...
	var parsePlayerConfig = function(uiConfConfig, callback){
		if(!uiConfConfig){
			if(This.client == null)
				This.initClient(KalturaConfig.config.client, function(){
					This.client.uiConf.get(function(result){
						uiConfConfig = KalturaUiConfParser.parseUiConfConfig(playerConfig['uiConfId'], result.config); //TODO validate result
						callback(uiConfConfig);
					}, playerConfig['uiConfId']);
				});	
		}
		else
			callback(uiConfConfig);		
	...
	KalturaCache.getMulti([cuePointsKey, elapsedTimeKey, entryRequiredKey, uiConfConfigKey], function(data){
		...
		parsePlayerConfig(uiConfConfig, function(vastConfig){
			if(!vastConfig.trackCuePoints){
				KalturaLogger.log('trackCuePoints is turned of for uiConf with id ' + playerConfig['uiConfId']);
				return;
			}

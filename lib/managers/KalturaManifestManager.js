...
KalturaManifestManager.prototype.master = function(request, response, params){
	...
	if(params.ui_conf){		
		if(!params.playerConfig){
			params.playerConfig = { 'uiConfId': params.ui_conf, 'partnerId': params.partnerId };
		}		
		else{
			params.playerConfig.uiConfId = params.ui_conf;
			params.playerConfig.partnerId = params.partnerId;
		}			
	}	
	...
	if(params.ui_conf){
		var uiConfConfigKey = KalturaCache.getUiConfConfig(params.ui_conf);
		KalturaCache.get(uiConfConfigKey, function(uiConfConfig) {
			if(!uiConfConfig){
				This.getAndStoreUiConfConfig(params.ui_conf, params.entryId, params.partnerId, function() {
					doFetchMaster();
				});
			}
			else{
...
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, entryId, sessionId, playerConfig, callback){
	...
	var getEntryAndMetadata = function(callback){
		var callMultiRequest = function(){
			This.impersonate(playerConfig.partnerId);
...
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, entryId, renditionId, masterUrl, sessionId){
	...
	KalturaCache.get(playerConfigKey, function(resPlayerConfig) {
		...
		KalturaCache.get(uiConfConfigKey, function(uiConfConfig) {
			if(!uiConfConfig){
				This.getAndStoreUiConfConfig(playerConfig.uiConfId, entryId, playerConfig.partnerId, function(uiConfConfig) {

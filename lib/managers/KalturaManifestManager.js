 ...
 * @param uiConfId
 */
KalturaManifestManager.prototype.getAndStoreUiConfConfig = function(uiConfId, entryId, callback){
	var callUiConfGetService = function(uiConfId){
		This.client.uiConf.get(function(result){
			...
			if(uiConfConfig && uiConfConfig.trackCuePoints){
				...
				KalturaCache.set(entryAdTrackRequiedKey, 1, KalturaConfig.config.cache.entryAdTrackRequied);
			}
...
KalturaManifestManager.prototype.master = function(request, response, params){
	var setEntryAdtrackRequired = function(shouldTrackAds, shouldTouch){
		if(shouldTrackAds){
	...
	if(params.ui_conf){		
		if(!params.playerConfig)
			params.playerConfig = { 'uiConfId': params.ui_conf };
		else
			params.playerConfig.uiConfId = params.ui_conf;
	}	
	...
	if(params.ui_conf){
		var uiConfConfoigKey = KalturaCache.getUiConfConfig(params.ui_conf);
		KalturaCache.get(uiConfConfoigKey, function(uiConfConfig) {
			if(!uiConfConfig){
				This.getAndStoreUiConfConfig(params.ui_conf, params.entryId, function(uiConfConfig) {
					setEntryAdtrackRequired(uiConfConfig.trackCuePoints, false);
					doFetchMaster();
				});
			}
			else{
				KalturaCache.touch(uiConfConfoigKey, KalturaConfig.config.cache.uiConfConfig);
				setEntryAdtrackRequired(uiConfConfig.trackCuePoints, true);
				doFetchMaster();
			}
		});
	}
 ...
 */
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, entryId, sessionId, playerConfig, callback){
	var This = this;
	var encodingIds = [];
	var mediaInfos = null;
	var entry = null;
		...
		KalturaAdIntegrationHandler.getAdMediaFiles(request, cuePoint, cachedUrl, entry, metadata, playerConfig, mediaInfos, sessionId, uiConfConfig, function(adFileId, adFileInfo){		
			...
			for(var i = 0; i < adFileInfo.mediaInfoIds.length; i++){
				if(adFileInfo.mediaInfoIds[i].trim().length){
					...
					stitchAdForEncodingId(cuePoint.id, encodingId, adFileId, adFileInfo.sharedFilePath);					
				}
			}				
		}, function(err){
	...
	var stitchAdForEncodingId = function(cuePointId, encodingId, adFileId, sharedFilePath){	
		var serverAdIdKey = KalturaCache.getServerAdId(cuePointId, encodingId);
...
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, entryId, renditionId, masterUrl, sessionId){
	...
	var finilizeResponse = function(manifestContent){
		response.writeHead(200, {'Content-Type': 'application/vnd.apple.mpegurl'});
	...
	var doHandleManifestContnet = function(manifestContent) {
		if(manifestContent){
			response.log('Rendition [' + renditionId + '] returned from cache');
			if(trackCuePoints){
				This.stitchCuePoints(request, response, entryId, sessionId, playerConfig, function(){
					finilizeResponse(manifestContent);
				});
			}
			else{
				finilizeResponse(manifestContent);
	...
	var handleManifestContnet = function(){
		if(trackCuePoints){
			manifestContentKey += '_' + This.manifestAdsExtension;
		}
		else{
			manifestContentKey += '_' + This.manifestNoAdsExtension;
		}
		
		KalturaCache.get(manifestContentKey, function(manifestContent) {
			touchRequiredSessionInfo();
			doHandleManifestContnet(manifestContent);
		});
	};
	...
	KalturaCache.get(playerConfigKey, function(resPlayerConfig) {
		...
		KalturaCache.get(uiConfConfigKey, function(uiConfConfig) {
			if(!uiConfConfig){
				This.getAndStoreUiConfConfig(playerConfig.uiConfId, entryId, function(uiConfConfig) {

...
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, entryId, callback){
	...
	var stitchAd = function(cuePoint) {
		...
		KalturaAdIntegrationHandler.getAdMediaFiles(cuePoint, entry, metadata, playerConfig, headers, mediaInfos, function(adId, adInfo){
			
			// TODO decide if cookie or cache should be used according to user-agent and ad-server host
			This.setCookie(response, cuePoint.id, adId, maxAge);
			KalturaCache.set(adId, adInfo, KalturaConfig.config.cache.adInfo);

			for(var i = 0; i < adInfo.encodingIds.length; i++){

				if(!adInfo.encodingIds[i].trim().length){
					continue;
				}
				
				var serverAdId = KalturaCache.getServerAdId(adId, adInfo.encodingIds[i]);
				...
				KalturaCache.touch(adMediaKey, KalturaConfig.config.cache.adMediaExtension, function(){
					...
					KalturaCache.add(adHandledKey, true, KalturaConfig.config.cache.adHandled, function(){
						var params = {
							...
							encodingId: adInfo.encodingIds[i],
							sharedFilePath: adInfo.sharedFilePath
						};
	...
	KalturaCache.get(manifestContentKey, function(data){
		if(data){
			...
			if(response.retries == 1){
				...
				This.fetchMaster(response, masterUrl, entryId, function(body, fromCache){
					var body = data.body;
					if(fromCache){

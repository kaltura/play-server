...
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, entryId, callback){
	...
	var stitchAd = function(cuePoint) {
		// TODO move counter to be encremented before all callbacks
		stitchAdCounter++;
		
		var sourceUrl = cuePoint.sourceUrl;
		// TODO - apply playerConfig on the sourceUrl
		
		// TODO use coffee and use the headers
		This.getHttpUrl(cuePoint.sourceUrl, function(xml){
			// TODO select best media file according to mediaInfos objects

			// TODO adId should be based on media URL and not XML URL.
			var adId = KalturaCache.getAdId(mediaUrl);

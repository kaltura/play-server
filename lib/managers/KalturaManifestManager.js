...
KalturaManifestManager.prototype.startWatcherExclusive = function(partnerId, entryId, manifestUrl, renditionsWatchParams) {
	...
	KalturaCache.add(entryRequiredKey, '\n', KalturaConfig.config.cache.entryHandled, function(){
		...
		KalturaLogger.log('Entry [' + entryId + '] url [' + manifestUrl + '] entry required is set, checking if watcher already exists');
		This.checkAndStartStreamWatcher(partnerId, entryId, manifestUrl, renditionsWatchParams);
	});
};
 ...
 * @param renditionsWatchParams
 */
KalturaManifestManager.prototype.checkAndStartStreamWatcher = function(partnerId, entryId, manifestUrl, renditionsWatchParams) {
	KalturaLogger.log('Entry [' + entryId + '] url [' + manifestUrl + '] validate stream watchers are up');
	var This = this;
	var renditionManifestHandledKeys = [];
	for(var i = 0; i < renditionsWatchParams.length; i++){
		renditionManifestHandledKeys.push(KalturaCache.getRenditionManifestHandled(renditionsWatchParams[i].url));
	}
	KalturaCache.getMulti(renditionManifestHandledKeys, function(data){
		for(var i = 0; i < renditionsWatchParams.length; i++){
			if(!data || !data[renditionManifestHandledKeys[i]]){
				KalturaLogger.log('Entry [' + entryId + '] manifestHandledKey [' + renditionManifestHandledKeys[i] + '] stream watcher is not running, starting');
				This.callPlayServerService('stream', 'watch', partnerId, renditionsWatchParams[i]);
			}			
		}		
	}, function(err){
		KalturaLogger.log('Entry [' + entryId + '] url [' + manifestUrl + '] failed to get renditionManifestHandledKeys, starting all stream watchers');
		for(var i = 0; i < renditionsWatchParams.length; i++){
			This.callPlayServerService('stream', 'watch', partnerId, renditionsWatchParams[i]);
		}
 ...
 * @param entryId
 */
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, partnerId, entryId, uiConfId, renditionId, masterUrl, sessionId, playerConfig){
	var This = this;

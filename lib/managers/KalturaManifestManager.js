...
KalturaManifestManager.prototype.master = function(request, response, params){
	...
	KalturaCache.set(
		...
		function(){
			KalturaLogger.log('successfully set headers [' + util.inspect(headers) +  '] for sessionId ' + sessionId);
		},
		function(){
			KalturaLogger.error('Failed to set headers for sessionId '+ sessionId)
		}
	);
...
KalturaManifestManager.prototype.replaceManifestTokens = function(manifestContent, sessionId, playerConfig, sessionStartTime, originDc){
	KalturaLogger.debug("Before replaceManifestTokens : "+ manifestContent);
	...
	KalturaLogger.debug("After replaceManifestTokens : "+ manifestContent);

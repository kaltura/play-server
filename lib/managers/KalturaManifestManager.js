	   		...
	   		for(var cuePointId in cuePoints){
				...
				var cuePointHandledKey = KalturaCache.getCuePointHandled(cuePointId, sessionId);
	   			if(cuePoint.triggeredAt && timeWindowStart < cuePoint.triggeredAt && cuePoint.triggeredAt < timeWindowEnd){
					KalturaCache.add(cuePointHandledKey, true, KalturaConfig.config.cache.cuePoint, stitchAd(cuePoint), function(err){
						KalturaLogger.log('cue point [' + cuePointId + '] for session [' + sessionId + '] already handeled');
					});
	   			}

...
var KalturaManifestManager = function(){
	try {
		KalturaManifestManager.RENDITION_MAX_RETRIES = parseInt(KalturaConfig.config.manifest.rendition.maxRetries);
	} catch (ex) {
...
KalturaManifestManager.prototype.master = function(request, response, params){
	...
	KalturaCache.set(
		...
		headers,
		86400,//saving it for one day
		function(){
			KalturaLogger.log('successfully set track info for sessionId ' + sessionId);
		},
		function(){
			KalturaLogger.error('Failed to set track info for sessionId '+ sessionId)
		}
	);
...
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, partnerId, entryId, uiConfId, uiConfConfigId, renditionId, masterUrl, sessionId, playerConfig, sessionStartTime, originDc){
	...
	if(response.retries >= KalturaManifestManager.RENDITION_MAX_RETRIES){
...
KalturaManifestManager.prototype.replaceManifestTokens = function(manifestContent, sessionId, playerConfig, sessionStartTime, originDc){
	KalturaLogger.log("Before replaceManifestTokens : "+ manifestContent);
	...
	KalturaLogger.log("After replaceManifestTokens : "+ manifestContent);

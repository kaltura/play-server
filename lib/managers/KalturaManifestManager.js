 ...
 * @returns string
 */
KalturaManifestManager.prototype.stitchMasterM3U8 = function(partnerId, entryId, uiConfId, manifestUrl, manifestContent) {
			...
			if(!response.headersSent){
				...
				if(fromCache){
		    		...
		    		KalturaCache.add(entryRequiredKey, ' ', KalturaConfig.config.cache.entryHandled, function(){
		    			...
		    			response.log('Entry [' + params.entryId + '] url [' + params.url + '] watcher already exists');
		    		});
				}
				else{
 ...
 */
KalturaManifestManager.prototype.stitchCuePoints = function(request, response, partnerId, entryId, uiConfConfig, sessionId){
	var This = this;
	var encodingIds = [];
	var mediaInfos = null;
	var entry = null;
	...
	KalturaCache.getMulti([cuePointsKey, elapsedTimeKey, entryRequiredKey, playerConfigKey], function(data){
		if(data[playerConfigKey]){
			KalturaCache.touch(playerConfigKey, KalturaConfig.config.cache.playerConfig, null, function() {
				KalturaLogger.log('Failed to load player config for session ' + sessionId + ' with error ' + err);
			});
			playerConfig = data[playerConfigKey];
		}
		else{
			KalturaLogger.log('player config not loaded for session ' + sessionId);
		}
 ...
 * @param entryId
 */
KalturaManifestManager.prototype.getRenditionFromCache = function(request, response, partnerId, entryId, uiConfId, renditionId, masterUrl, sessionId){
	if(response.retries){
		response.retries++;
	}
	else{
	...
	var uiConfConfigKey = KalturaCache.getUiConfConfig(uiConfId);
	...
	var handleManifestContent = function(uiConfConfig){
		...
		KalturaCache.get(manifestContentKey, function(manifestContent) {
			if(manifestContent){
				...
				if(trackCuePoints){
					This.stitchCuePoints(request, response, partnerId, entryId, uiConfConfig, sessionId);
				}
			}
			else{
				...
				if(response.retries == 1){
					...
					This.fetchMaster(response, masterUrl, entryId, function(data, fromCache){
						...
						if(fromCache){
							...
							body = This.stitchMasterM3U8(partnerId, entryId, uiConfId, masterUrl, body);
						}
					}, function(err){
						response.error(err);
					});
				...
				setTimeout(function(){
					...
					This.getRenditionFromCache(request, response, partnerId, entryId, uiConfId, renditionId, masterUrl, sessionId);
	...
	KalturaCache.get(uiConfConfigKey, function(uiConfConfigFromCache) {
		if(!uiConfConfigFromCache){
			This.getAndStoreUiConfConfig(uiConfId, entryId, partnerId, function(uiConfConfigResult) {
				if(uiConfConfigResult){
					setTrackCuePointsByUiConfig(uiConfConfigResult);
				}
				KalturaLogger.log('Failed to retrive uiConf config for uiConf id ' + uiConfId + ' session ' + sessionId + ' continuing with no ads');
				handleManifestContent(uiConfConfigResult);
			});
		}
		else{
			KalturaCache.touch(uiConfConfigKey, KalturaConfig.config.cache.uiConfConfig);
			setTrackCuePointsByUiConfig(uiConfConfigFromCache);
			handleManifestContent(uiConfConfigFromCache);
		}
	}, function(err) {
		KalturaLogger.log('Failed to load uiConfConfig for key ' + uiConfConfigKey + ' with error ' + err + ' continuing with default behavior');
...
KalturaManifestManager.prototype.rendition = function(request, response, params){
	...
	this.getRenditionFromCache(request, response, params.partnerId, params.entryId, params.uiConfId, params.renditionId, masterUrl, params.sessionId);
};

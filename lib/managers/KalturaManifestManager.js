 ...
 * @returns string
 */
KalturaManifestManager.prototype.stitchMasterM3U8 = function(partnerId, entryId, uiConfId, playerConfig, manifestUrl, trackCuePoints, manifestContent) {
		...
		This.fetchMaster(response, params.url, params.entryId, function(data, fromCache){
			if(!response.headersSent){
				...
				if(fromCache){
					...
					body = This.stitchMasterM3U8(params.partnerId, params.entryId, params.uiConfId, params.playerConfig, params.url, trackCuePoints, body); 
				}
		...
		KalturaCache.get(manifestContentKey, function(manifestContent) {
			if(manifestContent){
				...
				if(trackCuePoints){
					...
					This.callPlayServerService('cuePoints', 'stitch', partnerId, stitchParams);
				}
			}
			else{
				...
				if(response.retries == 1){
					...
					This.fetchMaster(response, masterUrl, entryId, function(data, fromCache){
						...
						if(fromCache){
							...
							body = This.stitchMasterM3U8(partnerId, entryId, uiConfId, playerConfig, masterUrl, trackCuePoints, body);
						}
					}, function(err){
						response.error(err);
					});
...
KalturaManifestManager.prototype.rendition = function(request, response, params){
	...
	this.getRenditionFromCache(request, response, params.partnerId, params.entryId, params.uiConfId, params.renditionId, masterUrl, params.sessionId, params.trackCuePoints, params.playerConfig);

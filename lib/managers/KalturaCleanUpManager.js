...
class KalturaCleanUpManager extends kaltura.KalturaManager {
	...
	dropEntryApiCachedResults(request, response, params)
	{
		...
		const getEntryCacheParams = { apiCallService: 'baseEntry', apiCallAction: 'get', params: [params.entryId], impersonatePartnerId: partnerId };
		const getEntryCacheKey = ApiServerClientConnector._generateCacheKey(getEntryCacheParams);
		KalturaCache.del(getEntryCacheKey,
			() => KalturaLogger.log(`Deleted entry get cache key for entry ${params.entryId} key ${getEntryCacheKey}`),
			(err) => KalturaLogger.warn(`Failed to delete entry get cache key for entry ${params.entryId}, key ${getEntryCacheKey}, due to ${util.inspect(err)}`)
		);

		const cueFilter = new client.objects.KalturaAdCuePointFilter();
		cueFilter.entryIdEqual = params.entryId;
		...
		KalturaCache.del(cuePointListCacheKey,
			() => KalturaLogger.log(`Deleted entry cue point list cache key for entry get ${params.entryId} key ${cuePointListCacheKey}`),
			(err) => KalturaLogger.warn(`Failed to delete entry cue point list cache key for entry ${params.entryId}, key ${cuePointListCacheKey}, due to ${util.inspect(err)}`)
		);

		const flavorAssetFilter = new client.objects.KalturaFlavorAssetFilter();
		flavorAssetFilter.entryIdEqual = params.entryId;
		...
		KalturaCache.del(flavorsListCacheKey,
			() => KalturaLogger.log(`Deleted entry flavor list cache key for entry get ${params.entryId} key ${flavorsListCacheKey}`),
			(err) => KalturaLogger.warn(`Failed to delete entry flavors list cache key for entry ${params.entryId}, key ${flavorsListCacheKey}, due to ${util.inspect(err)}`)
		);
	}

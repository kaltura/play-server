...
class KalturaLayoutManager extends kaltura.KalturaManager {
		...
		KalturaCache.get(layoutManifestKey,
			function (data)
			{
				if (data && data.length > 0)
				{
					...
					KalturaCache.touch(layoutManifestKey, This.layoutCacheTimeout,
						() => {KalturaLogger.debug(`managed to touch: ${layoutManifestKey} in cache`);},
						() => {KalturaLogger.debug(`failed to touch: ${layoutManifestKey} in cache`);}
					);
			...
			function(){
				...
				LayoutHelper.getEntryVersion(params.entryId,
					function(version){
						if(version)
						{
							KalturaCache.touch(params.entryId, KalturaConfig.config.cache.entryVersionTimeout,
								() => {KalturaLogger.debug(`managed to touch entry ${params.entryId} version in cache`);},
								() => {KalturaLogger.debug(`failed to touch entry ${params.entryId} version in cache`);}
							);
	...
	_handleSessionIdVesrion(params, versionId, request, response)
	{
		...
		{
			const sessionVersionKey = `${params.entryId}_${params.sessionId}`;
			LayoutHelper.initSessionVersion(sessionVersionKey, { id: versionId }, KalturaConfig.config.cache.sessionIdVersionTimeout,
				() => {
					...
					LayoutHelper.getSessionVersion(sessionVersionKey,
						(version) => {
							if(version)
							{
								KalturaCache.touch(sessionVersionKey, KalturaConfig.config.cache.sessionIdVersionTimeout,
									() => {KalturaLogger.debug(`managed to touch session: ${params.entryId}_${params.sessionId} version in cache`);},
									() => {KalturaLogger.debug(`failed to touch session: ${params.entryId}_${params.sessionId} version in cache`);}
								);
	...
	_processAdBreakReadyKeys(adBreakReadyKeys, handleJSONAdsCallback, errorCallback)
	{
		KalturaCache.getMulti(adBreakReadyKeys,
			function (value)
			{
				...
				KalturaCache.touchMulti(adBreakReadyKeys, KalturaConfig.config.cache.tinyUrlTtl,
					() => {KalturaLogger.debug(`managed to touch adBreakReadyKeys: ${adBreakReadyKeys} in cache`);},
					() => {KalturaLogger.debug(`failed to touch adBreakReadyKeys: ${adBreakReadyKeys} in cache`);}
				);

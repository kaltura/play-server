...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_createNoCuePointsManifestLayout(vodData)
	{
		...
		const contentClips = new SourceClipDataArray(0, vodData.getOnlySelectedFlavorPaths()).clips;
	...
	_createFullManifestLayout(vodData)
	{
		...
		for (let cueIndex = 0; cueIndex < filteredCuePoints.length; cueIndex++)
		{
			...
			else
			{
				...
				const contentClipArray = new SourceClipDataArray(contentOffset, vodData.getOnlySelectedFlavorPaths());
		...
		{
			const contentClips = new SourceClipDataArray(contentOffset, vodData.getOnlySelectedFlavorPaths());
	...
	adbreak(request, response, params, iteration = 1)
	{
		...
		this._getReadyAdsFromCache(adsReadyKey, adsDefaultReadyKey,
			...
			function ()
			{
				KalturaLogger.error(`Found no ads ready for ad break key ${adsReadyKey}, this probably means that the fetch warmup did not occur will initiate fetch and recall adbreak , iteration ${iteration}`);

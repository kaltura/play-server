...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_buildLayoutFromVodData(vodData, layoutManifestKey, partnerId, successCallback, errorCallback)
	{
		...
		else
		{
			...
			const fetchParams = { flavorIds: flavorIdsString };
			...
			this._shortenErrorManifestKeyTTL(layoutManifestKey, 'Layout created without ads');
		}
	...
	_shortenErrorManifestKeyTTL(manifestKey, message)
	{
		KalturaLogger.log(`Deleting manifest layout key ${manifestKey} from cache due to ${message}`);
		KalturaCache.touch(manifestKey, KalturaConfig.config.cache.errorLayout,
			() => KalturaLogger.log(`Shorten manifest key [${manifestKey}] ttl to ${KalturaConfig.config.cache.errorLayout} since found error or empty response`),
			(err) => KalturaLogger.error(`Failed to shorten manifest key [${manifestKey}] ttl due to ${util.inspect(err)}`)
		);
	}
	...
	_buildManifestAndResponse(response, partnerId, flavorIds, uiConfId, entryId, layoutManifestKey, headers, uriPrefixFormat)
	{
		...
		this.isPermissionAllowedForPartner(partnerId, 'FEATURE_PLAY_SERVER',
			function (isAllowed)
			{
					...
					This._getLayoutAPIInformation(partnerId, entryId, uiConfId, flavorIds, headers, uriPrefixFormat,
						function (vodData)
						{
							This._buildLayoutFromVodData(vodData, layoutManifestKey, partnerId,
								...
								function (err)
								{
									This._shortenErrorManifestKeyTTL(layoutManifestKey, 'Failed to build layout fro vodData');
						...
						function (err)
						{
							This._shortenErrorManifestKeyTTL(layoutManifestKey, 'Failed response from the API server');
				...
				{
					This._shortenErrorManifestKeyTTL(layoutManifestKey, 'not allowed for play server');
	...
	_getLayoutManifestKey(params)
	{
		...
		return KalturaUtils.getShortHashKey(layoutKey);
	}

...
/* global KalturaConfig KalturaLogger KalturaUtils KalturaCache AD*/
...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_allBlackFillersExistOnDisk(vodData)
	{
		for (const transcodingCommand of vodData.blackFillerTranscodeCommandList)
		{
			...
			KalturaLogger.log('XXX Validating this path exists as black filler ' + blackFillerPath);
			if (!KalturaFileUtils.checkFileExistsSync(blackFillerPath))
				return false;
		}
		KalturaLogger.log('XXX All black fillers existed on disk ');
		return true;
	}
		...
		function _parseApiResults(results)
		{
			...
			Promise.all(promises).then(
				function (promiseResults)
				{
					...
					for (let i = 0; i < flavorAssetList.length; i = i + 2)
					{
						...
						flavorAssetList[i].url = promiseResults[i];
						blackFillerTranscodingCommandList.push(promiseResults[i + 1]);
					}

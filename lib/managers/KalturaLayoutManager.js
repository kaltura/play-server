...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	static _getPrefix(flavorIds)
	{
		const flavors = flavorIds.split(',');
		let prefix = null;
		flavors.forEach(function(value){
			if (value.endsWith('_'))
				prefix = value;
		});
		return prefix;
	...
	{
		const prefix = KalturaLayoutManager._getPrefix(flavorIdsStr);
		if (!prefix)
			return flavorIdsStr.split(',');

		//remove the prefix from the list
		const flavors = flavorIdsStr.split(',');
		var index = flavors.indexOf(prefix);
		if (index > -1)

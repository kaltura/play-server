...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_getLayoutFromCache(response, layoutManifestKey, firstTry = true)
	{
		...
		KalturaCache.get(layoutManifestKey,
			function (data)
			{
				...
				{
					KalturaLogger.log(`Returning layout from cache: ${data}`);
	...
	_handleSessionIdVersion(params, versionId, request, response)
	{
		...
		if (ServerInfra.isParamDefined(params, 'sessionId'))
		{
			LayoutHelper.initSessionVersion(`${params.entryId}_${params.sessionId}`, {id: versionId}, KalturaConfig.config.cache.sessionIdVersionTimeout,
				() =>
				{
					const layoutManifestKey = LayoutHelper.getLayoutManifestKey(params.partnerId, params.entryId, params.flavorId, params.uiConfId, versionId);
	...
	_getLayoutAPIInformation(partnerId, entryId, uiConfId, selectedFlavorIds, headers, uriPrefixFormat, callback, errorCallback)
	{
		...
		function shouldIncludeFlavor(flavor, filteredTags)
		{
			...
			{
				if (tags.indexOf(filteredTags[j]) !== -1)
					return false;
			}
		...
		function getFilteredFlavors(flavorAssetList)
		{
			const result = [];
			const filteredTags = ['source', 'widevine', 'widevine_mbr', 'playready', 'ism', 'slweb'];
			for (let idx = 0; idx < flavorAssetList.length; idx++)
			{
				if (shouldIncludeFlavor(flavorAssetList[idx], filteredTags))
		...
		function _parseApiResults(entryResponse, cuePointListResponse, flavorAssetListResponse, uiConfResponse)
		{
			...
			flavorAssetList = getFilteredFlavors(flavorAssetList);
			const promises = [];
	...
	adbreak(request, response, params, didInnerFetchRun = false)
	{
		...
		function _handleAdBreak(adId)
		{
			...
			This._getReadyAdsFromCacheForFlavor(adsReadyKey, flavorId, didInnerFetchRun, true,
				...
				function (err)
				{
					...
					This.callPlayServerService('fetch', 'innerWarmup', params.partnerId, fetchParams, request.headers);
					setTimeout(
						function ()
		...
		AdBreakIdentifier.getAdBreakId(params.adId,
			_handleAdBreak,
			(err) =>
			{
				KalturaLogger.error(`Could not find ad id: ${params.adId} in cache due to ${err}`);
		...
		KalturaCache.get(adsReadyKey,
			function (allAdsReadyCacheValue)
			{
				...
				if (adBreakKeyHelper.areThereErrors())
				{
					...
					return LayoutHelper.processAdBreakReadyKeys([fillerKey], handleJSONAdsCallback, errorCallback);
				}
				...
				LayoutHelper.processAdBreakReadyKeys(adBreakReadyKeys, handleJSONAdsCallback, errorCallback);
			},
			function ()
			{
	...
	_createReadyAdBreakLayout(adReadyList, cuePointId, flavorId, duration)
	{
		...
		return JSON.stringify(adBreakLayout.toJSON());
	}
	...
	adpath(request, response, params)
	{
		...
		function _handleAdPath(adPathIdentifier)
		{
			...
			const jsonResponse = JSON.stringify(adPathLayoutData.toJSON());
	...
	static _getFlavorIdsFromUrl(origInput)
	{
		...
		const inputs = flavorIdsStr.split(/[\/,]/);
		const output = [];
		let i = 0;
	...
	/**
	 * @action playerManifest
	 * @param request
	 * @param response
	 * @param params
	 */
	playerManifest(request, response, params)
	{
		const This = this;

		response.dir(params);
		if (!this.validateActionArguments(params, ['entryId', 'flavorId', 'sessionId'], response))
			return;
		LayoutHelper.buildPlayerEntryLayout(params.partnerId, params.entryId, params.sessionId, params.flavorId, params.uiConfId,
			(jsonResponse) => This.okResponse(response, JSON.stringify(jsonResponse), 'text/plain'),
			(error) => This.errorResponse(response, 500, error)
		);
	}

	/**
	 * @action playerAdBreak
	 * @param request
	 * @param response
	 * @param params
	 */
	playerAdBreak(request, response, params)
	{
		const This = this;
		response.dir(params);
		...
		LayoutHelper.handlePlayerAdBreakLayoutRequest(params.adId, params.sessionId,
			(jsonResponse) => This.okResponse(response, JSON.stringify(jsonResponse), 'text/plain'),
			(error) => This.errorResponse(response, 500, error)
		);
	}

...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_buildLayoutFromVodData(vodData, layoutManifestKey, partnerId, successCallback, errorCallback)
	{
		let body;
		...
		else
		{
			...
			this._deleteManifestKey(layoutManifestKey, 'Layout created without ads');
		}
	...
	_deleteManifestKey(manifestKey, message)
	{
		...
		KalturaCache.del(manifestKey,
			() => KalturaLogger.log(`Managed to delete manifest key [${manifestKey}] that was calculated with error or empty response`),
	...
	_buildManifestAndResponse(response, partnerId, flavorIds, uiConfId, entryId, layoutManifestKey, headers, uriPrefixFormat)
	{
			...
			function (isAllowed)
			{
				if (isAllowed)
				{
					This._getLayoutAPIInformation(partnerId, entryId, uiConfId, flavorIds, headers, uriPrefixFormat,
						function (vodData)
						{
							This._buildLayoutFromVodData(vodData, layoutManifestKey, partnerId,
								function (body)
								{
								...
								function (err)
								{
									This._deleteManifestKey(layoutManifestKey, 'Failed to build layout fro vodData');
		...
		LayoutHelper.getEntryVersion(params.entryId,
			function(version){
				if(version)
				{
					...
					This._handleManifest(layoutManifestKey, request, response, params);
				}
				else
					This._handleFirstManifest(params,request,response);
			}, function(err){
				KalturaLogger.log(`could not get entry ${params.entryId} from cache , due to  ${util.inspect(err)}`);
				This._handleFirstManifest(params, request, response);
			}
		);
	}

	_handleFirstManifest(params, request, response)
	{
		const This = this;
		KalturaLogger.log(`Trying to add Entry ${params.entryId} version to cache with base version`);
		const baseLayoutManifestKey = This._getLayoutManifestKey(params, LayoutHelper.getBaseVersion());
		LayoutHelper.addEntryVersion(params.entryId,
			function(){
				//todo add log
				KalturaLogger.log(`First time handling layout manifest for entry ${util.inspect(params)}`);
				This._handleManifest(baseLayoutManifestKey, request, response, params);
			}, function(error){
				KalturaLogger.log(`could not add entry ${params.entryId} version to cache , due to  ${util.inspect(error)}`);
				This._getLayoutFromCache(response, baseLayoutManifestKey);
			}
		);
	}

	_handleManifest(layoutManifestKey, request, response, params)
	{
		const This = this;
		KalturaCache.add(layoutManifestKey, '', This.layoutCacheTimeout,
			function ()
			{
				...
				const flavors = KalturaLayoutManager._getFlavorIds(params.flavorIds);
				This._buildManifestAndResponse(response, params.partnerId, flavors, uiConfId, params.entryId, layoutManifestKey, request.headers, params.uriPrefixFormat);
			},
			function (err)
			{
	...
	_createNoCuePointsManifestLayout(vodData)
	{
		...
		const layout = new VODManifestLayoutData(vodData.selectedFlavorIdList);
	...
	static _getFlavorIds(flavorIdsStr)
	{
	...
	drop(request, response, params)
	{
		...
		CacheLock.lock(entryKey,
			() => { LayoutHelper.updateEntryVersion(entryKey,() => {
				...
				}, function(err){ KalturaLogger.log(`Could not update entry ${entryKey} version, due to : ${util.inspect(err)}`);

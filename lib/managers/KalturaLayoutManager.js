...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	manifest(request, response, params)
	{
		...
		LayoutHelper.addEntryVersion(params.entryId,
			function(){
				const baseVersion = LayoutHelper.getBaseVersion();
				const baseLayoutManifestKey = This._getLayoutManifestKey(params, baseVersion);
				KalturaLogger.log(`First time handling layout manifest for entry ${util.inspect(params)}`);
				This._handleManifest(baseLayoutManifestKey, request, response, params, baseVersion);
			}, function(error){
				...
				else
				{
					LayoutHelper.getEntryVersion(params.entryId,
						function(version){
							...
							KalturaLogger.log(`could not get entry ${params.entryId} version from cache , due to  ${util.inspect(err)}`);
							response.error(`No entry version found - Failed to create layout manifest though handled : ${err}`);
						}
					);
				}
			}

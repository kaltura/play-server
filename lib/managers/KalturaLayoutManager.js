...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_buildManifestAndResponse(response, partnerId, flavorIds, uiConfId, entryId, layoutManifestKey, headers)
	{
		...
		this.isPermissionAllowedForPartner(partnerId, 'FEATURE_PLAY_SERVER',
			function (isAllowed)
			{
				if (isAllowed)
				{
					This._getLayoutAPIInformation(partnerId, entryId, uiConfId, flavorIds, headers,
						function (vodData)
						{
							let body;
	...
	manifest(request, response, params)
	{
		...
		KalturaCache.add(layoutManifestKey, '', This.layoutCacheTimeout,
			function ()
			{
				...
				This._buildManifestAndResponse(response, params.partnerId, flavors, uiConfId, params.entryId, layoutManifestKey, request.headers);
			},
			function (error)
			{
	 ...
	 * @param callback
	 */
	_getLayoutAPIInformation(partnerId, entryId, uiConfId, selectedflavorIds, headers, callback, errorCallback)
	{
		const This = this;
		...
		function parseApiResults(results)
		{
			...
			for (let i = 0; i < flavorAssetList.length; i++)
				getFlavorPathPromises.push(This._getFlavorPath(flavorAssetList[i].id, partnerId, headers));
	 ...
	 * @private
	 */
	_getFlavorPath(flavorId, partnerId, headers)
	{
		return new Promise(

...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_createFullManifestLayout(vodData)
	{
		...
		for (let cueIndex = 0; cueIndex < filteredCuePoints.length; cueIndex++)
		{
			...
			const decodedFetchUrl = this._generateFetchUrl(fetchId);
			const fetchTime = Math.max(timeLineOffset - KalturaConfig.config.adIntegration.preFetchWindow, 0);
			layout.addNotification(new NotificationLayoutData(decodedFetchUrl, fetchTime));
			if (isPreRoll)
			{
				this._addAdbreakToLayout(layout, vodData, cuePoint, decodedFetchUrl);
	...
	adbreak(request, response, params, iteration = 1)
	{
		...
		if (!this.validateActionArguments(params, ['adId', 'sessionId', 'entryId'], response))
			return;
        const kalturaTinyUrl = kalturaTinyUrl();
        kalturaTinyUrl.load(params.adId,
        function(adId)
        {
	...
	{
        const decodedFatchId = KalturaTinyUrl.insert(fetchId);
        return `fetch/warmup/fetchId/${decodedFatchId}`;
	}

...
/* global KalturaConfig KalturaLogger KalturaUtils KalturaCacge */
...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	_buildManifestAndResponse(response, partnerId, flavorIds, uiConfId, entryId, layoutManifestKey, headers, drmFormat)
	{
		...
		this.isPermissionAllowedForPartner(partnerId, 'FEATURE_PLAY_SERVER',
			function (isAllowed)
			{
				if (isAllowed)
				{
					This._getLayoutAPIInformation(partnerId, entryId, uiConfId, flavorIds, headers, drmFormat,
						function (vodData)
						{
							let body;
	...
	manifest(request, response, params)
	{
		...
		KalturaCache.add(layoutManifestKey, '', This.layoutCacheTimeout,
			function ()
			{
				...
				This._buildManifestAndResponse(response, params.partnerId, flavors, uiConfId, params.entryId, layoutManifestKey, request.headers, params.drmFormat);
			},
			function (error)
			{
	...
	_getLayoutManifestKey(params)
	{
		...
		return KalturaCache.getShortHashKey(layoutKey);
	}

	_getLayoutAdBreakKey(params)
	{
		const layoutKey = `partnerId-${params.partnerId}-entryId-${params.entryId}-flavorIds-${params.flavorIds}-uiConfId-${params.uiConfId}`;
		return KalturaCache.getShortHashKey(layoutKey);
	}
	...
	_createFullManifestLayout(vodData)
	{
		...
		for (let cueIndex = 0; cueIndex < filteredCuePoints.length; cueIndex++)
		{
			...
			const encodedFetchId = this._generateFetchId(cuePoint.id, cuePoint.duration, cuePoint.sourceUrl, vodData.getAllFlavorIds(), fillerId);
			const url = this._generateFetchUrl(encodedFetchId);
	 ...
	 * @param callback
	 */
	_getLayoutAPIInformation(partnerId, entryId, uiConfId, selectedflavorIds, headers, drmFormat, callback, errorCallback)
	{
		const This = this;
		...
		function parseApiResults(results)
		{
			...
			for (let i = 0; i < flavorAssetList.length; i++)
				getFlavorPathPromises.push(This._getFlavorPath(flavorAssetList[i].id, partnerId, headers, drmFromat));
	 ...
	 * @private
	 */
	_getFlavorPath(flavorId, partnerId, headers, drmFormat)
	{
		return new Promise(
			function (resolve, reject)
			{
				...
				const url = PathsGenerator.generateApiServerFlavorURL(partnerId, flavorId, true, drmFormat);
	...
	adbreak(request, response, params, didInnerFetchRun = false)
	{
		...
		function _handleAdBreak(adId)
		{
			...
			This._getReadyAdsFromCacheForFlavor(adsReadyKey, flavorId, didInnerFetchRun, true,
				...
				function ()
				{
					...
					KalturaLogger.log(`Found no ads ready for ad break key ${adsReadyKey}, this probably means that the fetch warmup did not occur will initiate fetch and recall adbreak.`);
		...
		AdBreakIdentifier.getAdBreakId(params.adId, _handleAdBreak);
	}
	...
	_getReadyAdsFromCacheForFlavor(adsReadyKey, flavorId, shouldBlock, shouldGiveSecondChance, handleJSONAdsCallback, errorCallback)
	{
		...
		KalturaCache.get(adsReadyKey,
			function(allAdsReadyCacheValue)
			{
				...
				{
					KalturaLogger.error(`Could not find any ready ads for the constructed key : ${adsReadyKey}`);
					return errorCallback();
				}
				...
				KalturaCache.getMulti(adBreakReadyKeys,
					...
					function ()
					{
						KalturaLogger.error(`Failed to find ready ads for ads ${adBreakReadyKeys}`);
						errorCallback();
					},
					false
				);
		...
		for (let keyIdx = 0; keyIdx < keys.length; keyIdx++)
		{
			...
			if (adCacheDataObject.type === AD)
			{
				...
				for (let beaconIdx = 0; beaconIdx < adCacheDataObject.beaconList.length; beaconIdx++)
				{
					...
					const innerId = KalturaCache.getShortHashKey(`${keys[keyIdx]}-${beaconIdx}`);
	...
	_generateFetchUrl(encodedFetchId)
	{

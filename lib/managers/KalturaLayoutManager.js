...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	adbreak(request, response, params, iteration = 1)
	{
		...
		this._getReadyAdsFromCacheForFlavor(adsReadyKey, flavorId,
	...
	_getReadyAdsFromCacheForFlavor(adsReadyKey, flavorId, handleJSONAdsCallback, errorCallback)
	{
		KalturaCache.get(adsReadyKey,
			function(allAdsReadyList)
			{
				if (!allAdsReadyList || allAdsReadyList.length === 0)
				{
				...
				const adBreakKeyHelper = new AdBreakKeyHelper(allAdsReadyList);

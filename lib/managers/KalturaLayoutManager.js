...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	manifest(request, response, params)
	{
		...
		KalturaCache.add(layoutManifestKey, '', This.layoutCacheTimeout,
			function ()
			{
				...
				const flavors = KalturaLayoutManager._getFlavorIds(request.url);
	...
	static _getFlavorIds(origInput)
	{
		const flavorIdsStr = origInput.substr(origInput.indexOf("flavorIds/")+10);
		if (flavorIdsStr.indexOf('flavorId/') === -1)
		{
			let endOfFlavorsStr = flavorIdsStr.indexOf('/');
			if (!endOfFlavorsStr)
				endOfFlavorsStr = undefined;
			let flavors = flavorIdsStr.substr(0,endOfFlavorsStr).split(',');
			return flavors;
		}
		...
		KalturaLogger.debug('@@NA returning ['+util.inspect(output)+']');
		return output;
	}
}

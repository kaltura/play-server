...
class KalturaLayoutManager extends kaltura.KalturaManager {
															...
															engine.transcodeFile(commandLine).then(
																function (data)
																{
																	if(KalturaConfig.config.h264Verfication.enabled == 1)
																		verifier.verifyAd(flavorId, transcodedPath);
	 ...
	 * @param flavorId
	 */
	adbreak(request, response, params, isFirstRun=true)
	{
		const This = this;
		response.dir(params);
		...
		this._getReadyAdsFromCache(adsReadyKey,
			...
			function ()
			{
                ...
                if(!isFirstRun)
                {
                    response.error(`Could not create ads for entry id {$params.entryId} already retried once.`);
                    return;
                }

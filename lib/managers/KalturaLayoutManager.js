...
class KalturaLayoutManager extends kaltura.KalturaManager {
			...
			function(){
				...
				LayoutHelper.getEntryVersion(params.entryId,
					function(version){
						...
						KalturaLogger.log(`could not get entry ${params.entryId} version from cache , due to  ${util.inspect(getEntryErr)}`);
	...
	{
		const This = this;
		if(LayoutHelper.isParamDefined(params, 'sessionId'))
		{
			LayoutHelper.addSessionVersion(params.sessionId, { id: versionId }, KalturaConfig.config.cache.sessionIdVersionTimeout,
				() => {
					...
					LayoutHelper.getSessionVersion(params.sessionId,
						(version) => {
							if(version)
							{
							...
							KalturaLogger.log(`could not get version from cache to sessionId ${params.sessionId}, due to ${util.inspect(getSessionErr)}`);
							response.error(`could not get version from cache to sessionId`);
						}
					);
				});
		}
		else //version must be set
		{

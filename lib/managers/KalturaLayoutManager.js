...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	adbreak(request, response, params)
	{
		...
		//const adsReadyKey = KalturaFetchManager.generateAdsReadyCacheKey(cuePointId, flavorId, params.sessionId);
	...
	_getReadyAdsFromCache(adsReadyKey, handleJSONAdsCallback, errorCallback)
	{
		KalturaCache.get(adsReadyKey,
			...
			{
				KalturaLogger.log(`Retrieving the ad-Keys following from the cache ads list : ${adReadyListCacheValue}`);
				...
				KalturaCache.getMulti(adKeys,
					function (value)
					{
						KalturaLogger.log(`Got the following values from ad : ${JSON.stringify(value)}`);

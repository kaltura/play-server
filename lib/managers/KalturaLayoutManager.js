...
class KalturaLayoutManager extends kaltura.KalturaManager {
		...
		this.getClient(KalturaConfig.config.client,
			function ()
			{
				...
				for (let i = 0; i < flavorIds.length; i++)
					promises.push(This._getFlavorPath(flavorIds[i], partnerId));
	...
	adbreak(request, response, params, iteration = 1)
	{
		...
		const adsDefaultReadyKey = KalturaCache.getKey(KalturaCache.VOD_SESSION_DEFAULT_ADS_READY_KEY_PREFIX, [cuePointId, flavorId, params.sessionId]);

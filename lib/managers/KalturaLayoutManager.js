...
const APIRequestPromiseFactory = require('./helpers/APIRequestPromiseFactory');
	...
	_getLayoutAPIInformation(partnerId, entryId, uiConfId, selectedFlavorIds, headers, uriPrefixFormat, callback, errorCallback)
	{
		...
		function _parseApiResults(entryResponse, cuePointListResponse, flavorAssetListResponse, uiConfResponse)
		{
			...
			for (let i = 0; i < flavorAssetList.length; i++)
			{
				...
				promises.push(KalturaFFMpegCmdGenerator.generateCommandLineFormat(flavorAssetList[i].id, null, KalturaConfig.config.layout.fillerDefaultDurationSecs, This.apiConnector, partnerId, null));
			}
			for (let i=0; i < cuePointsList.length ; i++)
			{
				promises.push(APIRequestPromiseFactory.listCuePointMetadataPromise(KalturaConfig.config.layout.metadataProfileId, cuePointsList[i].id, partnerId));
			}
			Promise.all(promises).then(
				function (promiseResults)
				{
					...
					let j=0;
					for (let i=flavorAssetList.length * 2; i < (flavorAssetList.length * 2) + cuePointsList.length; i++)
					{
						if(promiseResults[i].objects[0].xml)
						{
							cuePointsList[j].overlay = true;
						}
						else
							cuePointsList[j].overlay = false;

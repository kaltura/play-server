...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	manifest(request, response, params)
	{
		...
		KalturaLogger.log('Moshe - '+ JSON.stringify(request));
	...
	_getLayoutAPIInformation(partnerId, entryId, uiConfId, selectedflavorIds, headers, uriPrefixFormat, callback, errorCallback)
	{
		...
		function parseApiResults(results)
		{
			...
			for (let i = 0; i < flavorAssetList.length; i++)
				getFlavorPathPromises.push(This._getFlavorPath(flavorAssetList[i].id, partnerId, headers, uriPrefixFormat));
		...
		function _handleAdBreak(adId)
		{
			...
			This._getReadyAdsFromCacheForFlavor(adsReadyKey, flavorId, didInnerFetchRun, true,
				...
				function (err)
				{
					...
					const fetchParams = { fetchId, sessionId: pa1rams.sessionId, entryId: params.entryId };
	...
	_createReadyAdBreakLayout(adReadyList, cuePointId, flavorId, duration)
	{
		...
		for (let keyIdx = 0; keyIdx < keys.length; keyIdx++)
		{
			...
			if (adCacheDataObject.type === AD)
			{
				...
				adBreakLayout.addClip(KalturaTinyUrl.insert(adCacheDataObject.path), durationForAd);
				const trackingList = [];
				// add all the beacons
		...
		if (leftDuration > 0 && foundFiller !== null)
		{
			while (leftDuration > 0)
			{
				...
				adBreakLayout.addClip(KalturaTinyUrl.insert(foundFiller.path), fillerCurrentDuration);
	...
	adpath(request, response, params)
	{
        ...
        KalturaTinyUrl.load(params.clipId,function(adPath)
        {
	    ...
	    },(err)=>KalturaLogger.info(`Layout add path failed  due to - ${err}`));
    }

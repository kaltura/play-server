	...
	handleManifest: function(manifestContent){
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(replacedByAd){
				...
				var preSegmentId = this.manager.cache.getPreSegmentId(this.entryId, this.renditionId);
				...
				for(var chunkIndex = 1; chunkIndex <= chunks; chunkIndex++){
					var adUrl = this.getPlayServerUrl('media', 'segment', {playerId: playerAdId, chunk: chunkIndex, renditionId: this.renditionId}) + '\n';
				...
				var postSegmentId = this.manager.cache.getPostSegmentId(this.entryId, this.renditionId);

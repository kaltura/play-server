...
KalturaStreamWatcher.prototype = {
	...
	stitchManifest: function(manifestContent){
		...
		var manifest = kaltura.m3u8Parser.parseM3U8(manifestContent, this.masterUrl);
		...
		for(var segmentIndex = 0; segmentIndex <= segmentsCount; segmentIndex++){
			...
			if(this.adCurOffset == 0){
				preSegmentStitchParams = this.buildPrePostSegmentStitchParams(buffer, false);
			}
			if(this.adCurOffset + curSegmentDuration <= this.adEndOffset && this.adCurOffset + curSegmentDuration + nextSegmentDuration > this.adEndOffset){
				postSegmentStitchParams = this.buildPrePostSegmentStitchParams(buffer, true);
			}
	...
	buildPrePostSegmentStitchParams: function(buffer, post){
		...
		var portion = null;
		if(post){
			segmentId = KalturaCache.getPostSegmentId(this.cuePoint.id, this.renditionId);
			offset = this.adEndOffset - this.adCurOffset;
			portion = 'right';
		}
		else{
			segmentId = KalturaCache.getPreSegmentId(this.cuePoint.id, this.renditionId);

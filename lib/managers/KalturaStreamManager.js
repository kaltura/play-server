		...
		for(var segmentIndex = 0; segmentIndex <= segmentsCount; segmentIndex++){
			...
			if(!this.inAdSlot && ((offsetStart && offsetEnd) || (timestampStart && timestampEnd))){
				...
				for(var cuePointId in this.cuePoints){
					...
					KalturaLogger.log('[' + This.uniqueLoopId + '] cuePoint [' + cuePointId + '] startTime [' + cuePoint.startTime + '] triggeredAt [' + cuePoint.triggeredAt + ']');
					...
					if( (cuePoint.startTime && offsetStart <= cuePoint.startTime && cuePoint.startTime <= offsetEnd) ||
						...
						KalturaLogger.log('[' + This.uniqueLoopId + '] Ad started cue-point[' + cuePointId + '] rendition [' + this.renditionId + '] start[' + this.adStartOffset + '] end[' + this.adEndOffset + ']');
	...
	parseEncodingParams: function(localPath){
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			KalturaCache.append(This.entryRequiredKey, '\n' + This.encodingId,
				function(){
					KalturaCache.get(This.entryRequiredKey,
						function(data){
							KalturaLogger.log('[' + This.uniqueLoopId + '] Entry required append occurred for key [' + This.entryRequiredKey + '] - now value is : ['  + JSON.stringify(data) + ']');
						},
						function(err){
							KalturaLogger.log('[' + This.uniqueLoopId + '] Entry required append no key found in cache for key [' +  This.entryRequiredKey + '] error was ' + err);
						});
				},
				function(err){
					KalturaLogger.log('[' + This.uniqueLoopId + '] Entry-Required key [' + This.entryRequiredKey + '] does not exist');

	...
	if(params.restored){
		...
		KalturaCache.get(this.renditionManifestHandledKey, function(data){
			if(!data){
				...
				KalturaCache.append(This.entryRequiredKey, entryRequiredValue, 
						function(){
	...
	verifyTrackingRequired: function(){
		...
		KalturaCache.get(this.entryRequiredKey, function(data){
			if(data){
				...
				var renditionIds = KalturaCache.extractEntryRequiredValue(data);
				var renditionRequired = false;
				...
				if(renditionRequired){
					...
					KalturaLogger.log('Rendition [' + This.renditionId + '] of Entry [' + This.entryId + '] not required any more');
					This.trackerRequired = false;					
				}				
			}
			else{

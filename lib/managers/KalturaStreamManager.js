...
KalturaStreamWatcher.prototype = {
	...
	verifyTrackingRequired: function(){
		...
		var cacheTouchErrorCallback = function(err){
			KalturaLogger.log(err + " Cache.Touch didn't succeed - setting trackerRequired to false.");
			This.trackerRequired = false;
		};
		...
		KalturaCache.get(this.entryRequiredKey, function(data){
			if(data && (data.indexOf(This.renditionId) != -1)){
				...
				KalturaCache.touch(This.mediaInfoKey, KalturaConfig.config.cache.encodingParams, null, cacheTouchErrorCallback);
				KalturaCache.touch(This.encodingParamsKey, KalturaConfig.config.cache.encodingParams, null, cacheTouchErrorCallback);
				KalturaCache.touch(This.fillerEncodingParamsKey, KalturaConfig.config.cache.fillerMedia, null, cacheTouchErrorCallback);
				KalturaCache.touch(This.uiConfConfigKey, KalturaConfig.config.cache.fillerMedia, null, cacheTouchErrorCallback);
			}
			else{
	...
	parseSegmentWhenRequired: function(currentSequence, firstSegmentsHistorySequence, lastSegmentsHistorySequence){
		...
		KalturaLogger.log('[' + This.uniqueLoopId + '] Before parseSegment rendition [' + this.renditionId + '] currentSequence [' + currentSequence + '] firstSegmentsHistorySequence [' + firstSegmentsHistorySequence 
				+ ']  lastSegmentsHistorySequence [' + lastSegmentsHistorySequence + '] elapsedTime [' + JSON.stringify(This.elapsedTime) + ']' + ' elapsedTimeSet [' + elapsedTimeSet + '] firstTime [' + This.firstTime + ']');
	...
	stitchManifest: function(manifestContent){
		...
		if(this.initSegmentsHistory(manifest, startSegment) === false
			...
			if (!this.encodingId  && !this.firstTime)
			{
				KalturaLogger.error('[' + this.uniqueLoopId + '] encodingId is null and it is not the first segment. Setting firstTime to true');
				this.firstTime = true;
			}
	...
	parseEncodingParams: function(localPath){
		var This = this;
		KalturaLogger.debug('[' + This.uniqueLoopId + '] parsing encoding params. Local path: [' + localPath + ']');
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			KalturaLogger.error('[' + This.uniqueLoopId + '] Error while parsing encoding params: ' + err);
			This.firstTime = true;
		});
	},

			...
			if(data){
				...
				for(var i = 0; i < renditionIds.length; i++){ 
					if(renditionIds[i].trim().length){
						var encodingParamsKey = KalturaCache.getEncodingParams(renditionIds[i]);
						var mediaInfoKey = KalturaCache.getMediaInfo(renditionIds[i]);
						var blackEncodingParamsKey = KalturaCache.getBlackEncodingParams(renditionIds[i]);

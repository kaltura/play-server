...
KalturaStreamWatcher.prototype = {
	...
	verifyTrackingRequired: function(){
		...
		KalturaCache.get(this.entryRequiredKey, function(data){
			if(data){
				...
				var fillerEncodingParamsKey = KalturaCache.getKey(KalturaCache.FILLER_ENCODING_PARAMS_KEY_PREFIX, [This.renditionId]);
				...
				KalturaCache.touch(fillerEncodingParamsKey, KalturaConfig.config.cache.fillerMedia);					
			}
			else{
	...
	stitchFiller: function(){
		var This = this;
		var fillerMediaKey = KalturaCache.getKey(KalturaCache.FILLER_MEDIA_KEY_PREFIX, [this.renditionId]);
		KalturaCache.touch(fillerMediaKey, KalturaConfig.config.cache.fillerMedia, function(){
			KalturaLogger.debug('Filler media [' + This.renditionId + '] already stitched');
			var fillerkEncodingParamsKey = KalturaCache.getKey(KalturaCache.FILLER_ENCODING_PARAMS_KEY_PREFIX, [This.renditionId]);
			KalturaCache.touch(fillerEncodingParamsKey, KalturaConfig.config.cache.fillerMedia);
		}, function(err){
			var fillerHandledKey = KalturaCache.getKey(KalturaCache.FILLER_HANDLED_KEY_PREFIX, [This.renditionId]);
			KalturaCache.add(fillerHandledKey, true, KalturaConfig.config.cache.fillerMedia, function(){
				KalturaLogger.log('Stitching filler media [' + This.renditionId + '] filler [' This.slateContent +']');
	...
	parseEncodingParams: function(localPath){
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			KalturaCache.set(fillerEncodingParamsKey, fillerEncodingParams, KalturaConfig.config.cache.fillerMedia);
		});
	},

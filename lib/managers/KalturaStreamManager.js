...
var KalturaStreamWatcher = function(manager, params, finishCallback){
	...
	this.lastSegmentTimeKey = KalturaCache.getLastSegmentTime(params.entryId);
	this.elapsedTime = {};
	this.cuePoints = {};
	...
	stitchManifest: function(manifestContent){
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(this.adCurOffset > this.adEndOffset){
				...
				var lastSegmentTime = {
						offset: this.segmentsHistory[buffer[0].sequence].offset,
						timestamp: this.segmentsHistory[buffer[0].sequence].timestamp,
					};
				
				KalturaCache.set(this.lastSegmentTimeKey, lastSegmentTime, KalturaConfig.config.cache.elapsedTime);				
			}
			else{

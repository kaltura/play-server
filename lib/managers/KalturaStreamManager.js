...
/**
 * Stream watcher, instantiated per flavor manifest
 * 
...
var KalturaStreamWatcher = function(manager, params, finishCallback){
	...
	this.renditionId = (params.bitrate + ':' + params.width + 'X' + params.height).md5();
	this.cuePoints = {};
...
KalturaStreamWatcher.prototype = {
	...
	handleManifest: function(manifestContent){
		...
		if(manifest.segments.length > 0){
			for(var i = 0; i < manifest.segments.length; i++){
				if(this.firstTime || this.lowestBitrate){
					...
					// TODO if it's the first segment, return and recall it from callback
				}
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(replacedByAd){
				...
				var preSegmentId = this.manager.cache.getPreSegmentId(cuePoint.id, this.renditionId);
				var playerAdId = this.manager.cache.getPlayerAdId(cuePoint.id, this.renditionId);
				newAds.push({
					playerId: playerAdId,
					cuePointId: cuePoint.id
				});

				this.stitchSegment(buffer, preSegmentId, preSegmentDuration, 'left');
				this.stitchBlack();
				...
				// TODO add the buffer only when it needed later
				var chunks = Math.ceil(cuePoint.duration / 10); // 10 seconds chunks
				for(var chunkIndex = 1; chunkIndex <= chunks; chunkIndex++){
					var adUrl = this.getPlayServerUrl('media', 'segment', {entryId: this.entryId, cuePointId: cuePoint.id, chunk: chunkIndex, renditionId: this.renditionId}) + '\n';
					var chunkDuration = Math.min(10000, cuePoint.duration - (chunkIndex * 10000));
					var adTranslation = {'URL' : adUrl, 'EXTINF' : chunkDuration};
				...
				var postSegmentId = this.manager.cache.getPostSegmentId(cuePoint.id, this.renditionId);
				
				this.stitchSegment(buffer, postSegmentId, postSegmentStartOffset, 'right');
		...
		// TODO verify that we don't insert ad if the sequence number already saved to cache
	...
	stitchBlack: function(){
		var This = this;
		var blackMediaKey = this.cache.getBlackMedia(this.renditionId);
		this.cache.get(blackMediaKey, function(data){
			if(data){
				KalturaLogger.log('Black media [' + this.renditionId + '] already stitched');
				return;
			}

			var blackHandledKey = This.cache.getBlackHandled(this.renditionId);
			This.cache.add(blackHandledKey, true, 60, function(){
				This.callPlayServerService('segment', 'stitchBlack', {renditionId: This.renditionId});
			}, function (err) {
				KalturaLogger.log('Black media [' + this.renditionId + '] already handled');
			});
		});
	},
	...
	getManifest: function(){
		...
		this.manager.cache.touch(this.flavorManifestHandledKey, 10);
	...
	parseEncodingParams: function(localPath){
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			var encodingParamsKey = This.manager.cache.getEncodingParams(This.renditionId);
			var blackEncodingParamsKey = This.manager.cache.getBlackEncodingParams(This.renditionId);
...
/**
 * Start watching flavor manifest
 * 
 * @action stream.watch
 * 
 * @param url

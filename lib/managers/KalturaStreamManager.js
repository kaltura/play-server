...
KalturaStreamWatcher.prototype = {
	...
	stitchManifest: function(manifestContent){
		...
		KalturaCache.get(this.elapsedTimeKey, function(elapsedTime){
			if(elapsedTime){
				for(var sequence in This.segmentsHistory){
					if(sequence === (elapsedTime.sequence - 1)){
						var segmentOffset = elapsedTime.offset + elapsedTime.duration;
						var segmentTimestamp = elapsedTime.timestamp + elapsedTime.duration;
						This.segmentsHistory[sequence].offset = segmentOffset;
						This.segmentsHistory[sequence].timestamp = segmentTimestamp;
						
						elapsedTime = {
							sequence: sequence,
							duration: This.segmentsHistory[sequence].duration,
							offset: segmentOffset,
							timestamp: segmentTimestamp
						};
					}
				}
				KalturaCache.set(This.elapsedTimeKey, elapsedTime, KalturaConfig.config.cache.elapsedTime);
			}
		});
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(this.urlTranslations[segmentUrl]){
				KalturaLogger.debug('Translating [' + segmentUrl + '] to [' + this.urlTranslations[segmentUrl].url + ']');
			...
			KalturaLogger.debug('Stitch segment [' + buffer[1].sequence + '] URL [' + segmentUrl + '] start [' + offsetStart + '] end [' + offsetEnd + ']');
			...
			if(!this.lastUsedSegment || buffer[0].sequence > this.lastUsedSegment){
				...
				if(!this.inAdSlot && offsetStart && offsetEnd){
	...
	parseEncodingParams: function(localPath){
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			encodingParams = kaltura.ffmpegParams.buildEncodingParams(mediaInfo, true, true);
			blackEncodingParams = kaltura.ffmpegParams.buildBlackInputParams(mediaInfo);
			encodingParamsForBlack = kaltura.ffmpegParams.buildEncodingParams(mediaInfo, false, false);
			...
			KalturaCache.set(blackEncodingParamsKey, blackEncodingParams + ' ' + encodingParamsForBlack, KalturaConfig.config.cache.encodingParams);
		});
	},

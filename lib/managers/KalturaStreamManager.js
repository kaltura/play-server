...
KalturaStreamWatcher.prototype = {
	...
	stitchBlack: function(){
		...
		KalturaCache.touch(blackMediaKey, function(){
			...
			KalturaCache.add(blackHandledKey, true, 60, function(){
	...
	getManifest: function(){
		KalturaCache.set(This.renditionManifestHandledKey, true, KalturaStreamWatcher.RESULT_MANIFEST_EXPIRY);
		...
		this.manager.getHttpUrl(this.url, function(manifestContent){
			...
			KalturaCache.set(This.trackerOutputKey, newManifestContent, KalturaStreamWatcher.RESULT_MANIFEST_EXPIRY);
		}, function(err){
	...
	parseEncodingParams: function(localPath){
		...
		if(KalturaConfig.config.bin.mediaInfoPath){
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			KalturaCache.set(encodingParamsKey, encodingParams, 600);
			KalturaCache.set(blackEncodingParamsKey, blackEncodingParams, 600);
		});
	...
	handleSyncPoint: function(entryId, segment, syncPoint){
		...
		KalturaCache.set(this.elapsedTimeKey, elapsedTime, 600);
	},

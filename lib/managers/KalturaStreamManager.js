		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(buffer[0].offset || buffer[1].offset){
				for(var cuePointId in this.cuePoints){
					...
					if(buffer[0].offset <= cuePoint.startTime && cuePoint.endTime <= offsetEnd){
						partOfAd = true;
						break;
					}
				}
			...
			if(buffer[0].offset <= cuePoint.startTime && cuePoint.startTime < buffer[1].offset){
				...
				var tempClipUrl = this.manager.getPlayServerUrl('media', 'segment', {segmentId: preSegmentId}) + '\n';
				urlTranslations[url2] = {'URL' : tempClipUrl, 'EXTINF' : segmentLength};
				newResult.push(urlTranslations[url2]);
			}
			else if(buffer[1].offset <= cuePoint.startTime && cuePoint.startTime < buffer[2].offset){
//				TODO
//				# ad ends here
	...
	parseSegment: function(entryId, segment){
		...
		var request = http.request(options, function(response) {
			if (response.statusCode != 200) {
				KalturaLogger.error('Parse segment entry [' + entryId + '] segment [' + segment.sequence + ']: Invalid http status: ' + response.statusCode);
				return;
			}
...
//					this.manager.cache.append(ffmpegParamsKey, tsEncodingParams + '\n', 30);
				...
				if(This.lowestBitrate){
					This.parseSyncPoints(This.entryId, segment, Buffer.concat(buffers));
				}
			});
		});

		request.on('error', function(e) {
			KalturaLogger.error('Parse segment entry [' + entryId + '] segment [' + segment.sequence + ']: http error: ' + e.message);
		});

		request.end();
	},

	parseSyncPoints: function(entryId, segment, buffer){
		KalturaLogger.log('Parse sync-points entry [' + entryId + '] segment [' + segment.sequence + ']');

...
KalturaStreamWatcher.prototype = {
	...
	parseSegment: function(entryId, segment, numberOfSegments){
		...
		var request = http.request(options, function(response) {
			...
			if(This.firstTime || KalturaLogger.largeDataDebugEnabled){				
				...
				KalturaLogger.log('Saving segment entry [' + entryId + '] segment [' + segment.sequence + ']  url [' + segment.resolvedUrl + '] in [' + localPath +']');
	...
	parseEncodingParams: function(localPath){
		...
		kaltura.mediaInfo.parse(localPath, function(mediaInfo){
			...
			KalturaLogger.debug('Encoding params for rendition [' + This.renditionId + '] and encodingId [ ' + This.encodingId + '] from file [' + localPath + ']: ' + encodingParams);
			KalturaCache.append(This.entryRequiredKey, '\n' + This.encodingId,
				function(){
					KalturaCache.get(This.entryRequiredKey,
						function(data){
							KalturaLogger.log('Entry required append occurred for key [' + This.entryRequiredKey + '] - now value is : ['  + JSON.stringify(data) + ']');
						},
						function(err){
							KalturaLogger.log('Entry required append no key found in cache for key [' +  This.entryRequiredKey + '] error was ' + err);
						});
				},
				function(err){

	...
	handleManifest: function(manifestContent){
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(replacedByAd){
				...
				var adPlayerId = this.manager.cache.getPlayerAdId(this.entryId, this.renditionId);
				newAds.push({
					playerId: adPlayerId,
				...
				for(var chunkIndex = 1; chunkIndex <= chunks; chunkIndex++){
					var adUrl = this.getPlayServerUrl('media', 'segment', {playerId: adPlayerId, chunk: chunkIndex, renditionId: this.renditionId}) + '\n';

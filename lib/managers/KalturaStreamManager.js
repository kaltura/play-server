...
KalturaStreamWatcher.prototype = {
	...
	stitchManifest: function(manifestContent){
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			var offset = this.segmentsHistory[buffer[1].sequence].offset;
			offsetStart = offset ? offset : null;
			...
			if(!this.lastUsedSegment || buffer[0].sequence > this.lastUsedSegment){
				...
				if(!this.inAdSlot && offsetStart && offsetEnd){
					for(var cuePointId in this.cuePoints){
						...
						KalturaLogger.debug('cuePoint [' + cuePointId + '] startTime[' + cuePoint.startTime + ']');
						...
						if(offsetStart <= cuePoint.startTime && cuePoint.startTime <= offsetEnd){
							...
							this.adSegmentsOffset = {};
							this.adSegmentsOffset[buffer[1].sequence] = 0;
			...
			var adCurOffset = 0;
			if(buffer[0].sequence > this.adStartSegment)
				adCurOffset = this.adSegmentsOffset[buffer[0].sequence - 1];
			...
			if(adCurOffset + curSegmentDuration <= this.adEndOffset && adCurOffset + curSegmentDuration + nextSegmentDuration > this.adEndOffset){
				// create post ad ts
				var postSegmentId = KalturaCache.getPostSegmentId(this.cuePoint.id, this.renditionId);
				this.stitchSegment(buffer, postSegmentId, this.adEndOffset - adCurOffset, 'right');
			}

			if(adCurOffset > this.adEndOffset){
				// last segment
				outputEnd = 0;
			}
			else{
				outputEnd = adCurOffset + curSegmentDuration;
			}
			...
			if(adCurOffset > this.adEndOffset){
				...
				adCurOffset += curSegmentDuration;
				this.adSegmentsOffset[buffer[0].sequence] = adCurOffset;
			}
		}

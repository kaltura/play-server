	...
	handleManifest: function(manifestContent){
		...
		for(var segmentIndex = 0; segmentIndex < segmentsCount; segmentIndex++){
			...
			if(replacedByAd){
				...
				var preSegmentId = this.manager.cache.getPreSegmentId(this.entryId, cuePoint.id, this.renditionId);
				var playerAdId = this.manager.cache.getPlayerAdId(this.entryId, cuePoint.id, this.renditionId);
				newAds.push({
					playerId: playerAdId,
				...
				var postSegmentId = this.manager.cache.getPostSegmentId(this.entryId, cuePoint.id, this.renditionId);

...
var KalturaStreamWatcher = function(manager, params, finishCallback){
	...
	this.masterManifestRequiredKey = KalturaCache.getMasterManifestRequired(params.masterUrl);
		...
		KalturaCache.get(this.renditionManifestHandledKey, function(data){
			if(!data){
				...
				KalturaCache.append(This.entryRequiredKey, entryRequiredValue, function(){
					...
					KalturaLogger.log('Failed to append [' + This.entryRequiredKey + '] :' + err);
					KalturaCache.add(This.entryRequiredKey, entryRequiredValue, KalturaConfig.config.cache.entryHandled);
				});
				This.getManifest();
			}
		});
...
KalturaStreamWatcher.prototype = {
	...
	verifyTrackingRequired: function(){
		var This = this;
		KalturaCache.getMulti([this.entryRequiredKey, this.masterManifestRequiredKey], function(data){
			if(data[This.masterManifestRequiredKey]){
				KalturaLogger.debug('Rendition [' + This.renditionId + '] entry [' + This.entryId + '] still required [' + data[This.entryRequiredKey] + ']');
				This.trackerRequired = true;
				var renditionIds = KalturaCache.extractEntryRequiredValue(data[This.entryRequiredKey]);
				...
				var encodingParamsKey = KalturaCache.getEncodingParams(This.renditionId);
				var mediaInfoKey = KalturaCache.getMediaInfo(This.renditionId);
				var blackEncodingParamsKey = KalturaCache.getBlackEncodingParams(This.renditionId);
	...
	stitchManifest: function(manifestContent){
		...
		for(var segmentIndex = 0; segmentIndex <= segmentsCount; segmentIndex++){
			...
			var tokens = {sessionId: '@SESSION_ID@', sessionStartTime: '@SESSION_START_TIME@'};
	...
	stitchBlack: function(){
		var This = this;
		var blackMediaKey = KalturaCache.getBlackMedia(this.renditionId);
		KalturaCache.touch(blackMediaKey, KalturaConfig.config.cache.blackMedia, function(){
			...
			var blackHandledKey = KalturaCache.getBlackHandled(This.renditionId);
	...
	removeFromEntryRequired: function(entryRequiredKey, entryId, renditionId){
		KalturaLogger.log('Removing [' + renditionId + '] from entryRequired [' + entryId + ']');
		KalturaCache.get(entryRequiredKey, function(data){
			if(data){
				var updatedEntryRequiredValue = data.replace(new RegExp( KalturaCache.buildEntryRequiredValue(renditionId), "g" ), '');
				KalturaCache.set(entryRequiredKey, updatedEntryRequiredValue, KalturaConfig.config.cache.entryHandled, function(){
					KalturaLogger.log('Updated entryRequired for entry [' + entryId + '] is [' + updatedEntryRequiredValue + ']');
				}, function(err){
					KalturaLogger.error('Failed to update entryRequired for entry [' + entryId + '] rendition [' + renditionId + ']');
				});
			}
		});
	...
	getManifest: function(){
		...
		KalturaLogger.log('[' + this.uniqueLoopId + '] getting manifest for [' + this.entryId + '] rendition [' + this.renditionId + ']');
		...
		if(new Date().getTime() > (this.startTime + KalturaStreamWatcher.MINIMUM_RUN_PERIOD) && !this.trackerRequired){
			...
			this.removeFromEntryRequired(this.entryRequiredKey, this.entryId, this.renditionId);
	...
	parseSegment: function(entryId, segment, numberOfSegments){
		KalturaLogger.log('[' + this.uniqueLoopId + '] Parse segment entry [' + entryId + '] segment [' + segment.sequence + '] url [' + segment.resolvedUrl + ']');
		
		if(this.lastParsedSegment >= segment.sequence && (this.lastParsedSegment - segment.sequence) <= numberOfSegments){
			KalturaLogger.debug('[' + this.uniqueLoopId + '] Exit parse segment, lastParsedSegment: [' + this.lastParsedSegment +']');
			return;
		}
		...
		var request = httpModule.request(options, function(response) {
			if (response.statusCode != 200) {
				KalturaLogger.error('[' + This.uniqueLoopId + '] Parse segment entry [' + entryId + '] segment [' + segment.sequence + '] url [' + segment.resolvedUrl + ']: Invalid http status: ' + response.statusCode);
				return;
			}
			...
			if(This.firstTime || KalturaLogger.largeDataDebugEnabled){				
				...
				KalturaLogger.log('[' + This.uniqueLoopId + '] Saving segment entry [' + entryId + '] segment [' + segment.sequence + ']  url [' + segment.resolvedUrl + '] in [' + localPath +']');
		...
		request.on('error', function(e) {
			KalturaLogger.error('[' + This.uniqueLoopId + '] Parse segment entry [' + entryId + '] segment [' + segment.sequence + '] url [' + segment.resolvedUrl + ']: http error: ' + e.message);
		});

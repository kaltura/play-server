						...
						case FILE_STATUS.DOESNT_EXIST:
							KalturaUtils.createFilePath(response.path,
								function ()
								{
									try
									{
										fs.writeFileSync(`${response.path}.${TMP_FILE_SUFFIX}`, EMPTY_STRING, { flag: 'wx' });
										KalturaLogger.log(`created tmp file ${response.path}.${TMP_FILE_SUFFIX}`);
									...
									{
										reject(`Failed to create tmp file ${response.path}.${TMP_FILE_SUFFIX} , due to ${ex}`);
									}
								},
								function (err)
								{

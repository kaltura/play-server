	...
	static getAdaptedTranscodingCommand(flavorId, jsonInfo, durationInSeconds, apiConnector, partnerId, inPath, outFilePrefixPath, isAd = false)
	{
		return new Promise(
			function (resolve, reject)
			{
				...
				{
					KalturaLogger.error(`Failed to generate transcoding command for path: ${inPath} , due to : ${err}`);
					reject(err);
				}
				...
				function tryTranscodeFile(path, command)
				{
					...
					try
					{
						...
						gTranscodingEngine.transcodeFile(command, flavorId, path).then(
							...
							function (err)
							{
								reject(`Failed to transcode file to path ${path} due to error: ${err}`);
							}
						);
					}
					catch (ex)
					{
						reject(`Failed to create tmp file ${tmpFilePath} , due to ${ex}`);
					}
				}
						...
						switch (status)
						{
							...
							case FILE_STATUS.DOESNT_EXIST:
								KalturaUtils.createFilePath(response.path,
									...
									(err) => reject(`Failed to create filePath to path ${response.path} ,due to error: ${err}`)
								);
								break;
							default:

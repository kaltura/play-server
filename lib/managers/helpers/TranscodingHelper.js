...
class TranscodingHelper {
	...
	static getAdaptedTranscodingCommand(flavorId, jsonInfo, durationInSeconds, apiConnector, partnerId, inPath, outFilePrefixPath, isAd = false, cuePointOverlay = null, cuePointId = null)
	{
		return new Promise(
			function (resolve, reject)
			{
				...
				let sendCuePoint = null;
				if(cuePointOverlay)
					sendCuePoint = cuePointId;
				KalturaFFMpegCmdGenerator.generateCommandLineFormat(flavorId, jsonInfo, durationInSeconds, apiConnector, partnerId, sendCuePoint).then(
					successfullyGeneratedCommand, failedToGenerateCommand);
			}
		);
	}

	static transcodeAndSaveToDisk(flavorId, jsonInfo, durationInSeconds, apiConnector, partnerId, inPath, outDir, isAd = false, cuePointOverlay = null, cuePointId = null)
	{
		return new Promise(
			function (resolve, reject)
			{
				...
				TranscodingHelper.getAdaptedTranscodingCommand(flavorId, jsonInfo, durationInSeconds, apiConnector, partnerId, inPath, outDir, isAd, cuePointOverlay, cuePointId).then(
					gotCommandResponse, reject);
		});
	}

	static transcodeExistingFileToDisk(flavorId, apiConnector, partnerId, inPath, outDir, durationInSeconds = null, isAd = false, cuePointOverlay = null, cuePointId = null)
	{
		return new Promise( function(resolve,reject)
		{
			function parsedMediaInfo(mediaInfo)
			{
				...
				TranscodingHelper.transcodeAndSaveToDisk(flavorId, mediaInfo.jsonInfo, durationInSeconds, apiConnector, partnerId, inPath, outDir, isAd, cuePointOverlay, cuePointId).then(

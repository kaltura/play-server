...
class AdBreakKeyHelper {
	...
	_parseAdTag(adTagValueSplit, readyAdsBreakData)
	{
		...
		try {
			...
			throw new Error(`Unexpected values found in cache for the filler value: ${util.inspect(adTagValueSplit)}`);
		}
	}
	...
	static blockKey(adsReadyKey)
	{
		KalturaCache.append(adsReadyKey, `${READY_ADS_SIGN_SEPARATOR}${BLOCKED}${READY_ADS_SIGN_SEPARATOR}`,
			...
			function (err)
			{
				throw new Error(`Failed to block ready ads, existing key ${adsReadyKey}, due to ${err}`);
			}
		);
	}
	...
	static addAdOrFiller(adsReadyKey, adCacheData, value)
	{
		...
		KalturaCache.append(adsReadyKey, keyToAppend,
			...
			function (err)
			{
				throw new Error(`Failed to append ad key to ready ad key: ${adsReadyKey} due to: ${err}`);
			}
		);
	}
	...
	static getErrorCacheValue(error)
	{
		return `${ERROR}${READY_ADS_INNER_DATA_SIGN_SEPARATOR}${error}`;
	}

	getAdBreakReadyKeysForFlavor(flavorId)
	{
		// TODO change this function to return a subset instead of all or nothing
		let shouldReturnAds = true;
		...
		for (let i = 0; i < this.readyAdsBreakData.flavorIds.length; i++)
		{
			...
			if (readyAdsIndexes.length !== this.readyAdsBreakData.getExpectedNumOfAds())
		...
		let answer = [];
		if (shouldReturnAds)
			answer = answer.concat(givenFlavorReadyAdsBreakData.getReadyAdsKeys());

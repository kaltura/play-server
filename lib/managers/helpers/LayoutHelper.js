...
class LayoutHelper
{
		...
		else
		{
			LayoutHelper.getEntryVersion(entryId ,
				function (version)
				{
					...
					{
						KalturaLogger.error(`Failed to get and update Entry ${entryId} version , this probably means that key ${entryId} doesnt exist in cache`);
						errorCallback();
					}
				},
				function (err)
				{
					KalturaLogger.error(`Failed to get and update Entry ${entryId} version , due to ${err}`);
					errorCallback();
				}
			);
		}
	 ...
	 * @param errorCallback
	 */
	static initSessionVersion(sessionId, version, entryTimeOut, successCallback, errorCallback)
	{
		KalturaCache.add(sessionId, version , entryTimeOut, successCallback, errorCallback);
	}

	static getSessionVersion(sessionId, successCallback, errorCallback)
	{
		KalturaCache.get(sessionId, successCallback, errorCallback);
	}
}

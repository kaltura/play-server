		...
		else
		{
			LayoutHelper.getEntryVersion(entryId ,
				function (version)
				{
					...
					{
						KalturaLogger.info(`Entry Id ${entryId} did not exist in cache - not updating cache`);
						successCallback();
					}
				},
				function (err)
				{
					KalturaLogger.error(`Failed to get and update Entry ${entryId} version , due to ${util.inspect(err)}`);
					errorCallback();
				}
			);
		}

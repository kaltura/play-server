...
class LayoutHelper
{
	...
	static manifestToPlayerLayoutTranslator(manifestText, preRollPlayerAdDataArray)
	{
		...
		{
			KalturaLogger.error(`Failed to parse manifest text as json , manifest text got was :${manifestText}`);
			manifestValue = null;
		}
	...
	static transformMultiSJsonAdsToPlayerAdBreak(adReadyList, duration, cuePointId, flavorId)
	{
		...
		{
			const playerAdData = new PlayerAdData(0, currentOffset, leftDuration);

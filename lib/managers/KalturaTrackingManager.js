...
class KalturaTrackingManager extends kaltura.KalturaManager
{
	...
	{
		let keyToHash = `${params.partnerId}-${params.entryId}-${params.cuePointId}-${params.url}-${JSON.stringify(params.headers)}-${params.uniqueId}`;
	...
	sendBeacon(request, response, params)
	{
		...
		KalturaLogger.log(`Tracking info as string ${trackingInfoString}`);
		...
		trackingInfo.trackingId = `tracking-${params.entryId}-${trackingInfo.cuePointId}-${trackingInfo.type}`;

...
let requestPckg = require('request');
requestPckg = requestPckg.defaults({ timeout: parseInt(KalturaConfig.config.cloud.requestTimeout) * 1000 });
/**
 * @service tracking
 */
class KalturaTrackingManager extends kaltura.KalturaManager
{
	...
	static buildTrackingBeaconCacheKey(trackingPrefix, partnerId, entryId, cuePointId, url, sessionId, innerId)
	{
		...
		const md5 = KalturaUtils.getShortHashKey(keyToHash);
	...
	sendBeacon(request, response, params)
	{
		...
		if (!this.validateActionArguments(params, ['trackingId', 'entryId', 'partnerId', 'sessionId'], response))
			return;
		// we return ok response instantly to avoid waits/recalls from the NGINX
		This.okResponse(response, 'OK', 'text/plain');
		function _handleTrackingId(trackingIdentifier)
		{
			KalturaLogger.log(`Got tracking info ${util.inspect(trackingIdentifier)}`);
        ...
        TrackingIdentifier.getTrackingId(params.trackingId,
										_handleTrackingId,
										(err)=>KalturaLogger.info(`Beacon was not send due to - ${util.inspect(err)}`));
	}
	...
	static calculateBeaconOffset(type, initialOffsetMsecs, durationMsecs)
	{
		switch (type)
		{
			...
			case 'complete':
				return Math.floor(initialOffsetMsecs + (durationMsecs * 95 / 100));
			default :

...
class KalturaTrackingManager extends kaltura.KalturaManager
{
    ...
    sendBeacon(request, response, params)
    {
        ...
        response.log('Sending tracking beacon for trackingId: [' + params.trackingId + ' partnerId: [' + params.partnerId + '] entryId: [' + params.entryId + '] cuePointId: [' + params.cuePointId+ '] beaconUrl: [' + beaconUrl + ']' + ' type: [' + params.type + ']');
    ...
    handleTrackingBeacon(request, response, params, beaconUrl)
    {
        ...
        KalturaCache.get(trackingBeaconCacheKey, function (trackingBeaconItem) {
            ...
            {
                response.log('Tracking info deosn\'t exist in cache. Continue sending beacon request.');
            ...
            response.log('Tracking info deosn\'t exist in cache. Continue sending beacon request.' + err);
            This.sendBeaconForType(response, beaconUrl, params);
            KalturaCache.set(trackingBeaconCacheKey, true, KalturaConfig.config.cache.trackingBeaconUrl);
        });
    }
    ...
    {
        response.log('Start sending beacon request for beaconUrl: [' + url + ' type: [' + params.type + '] tracking id: [ '+ params.trackingId + '] partnerId: [' + params.partnerId + ']');
        if (!url)
        {
            response.log('Failed to send beaconUrl[' + url + ' type [' +params.type + '] tracking id: [ '+ params.trackingId + '] partner [' + params.partnerId + '], empty url');
            return;
        }
        ...
        let request = httpModule.get(url, function (res) {
            ...
            {
                response.log('Failed to send beacon for type [' + params.type + '] tracking id: [ '+ params.trackingId + '] partnerId [' + params.partnerId + '] url [' + url + '], timeout');
                msgobj.status = 'Failed';
            }
            else
            {
                response.log('Beacon for type [' + params.type + '] tracking id: [ '+ params.trackingId + '] partnerId [' + params.partnerId + '] url [' + url + '] sent with status: [' + res.statusCode + ']');
        ...
        request.on('error', function (e) {
            if (!responseEnded)
                response.log('Failed to send beacon for type [' + params.type + '] tracking id: [ '+ params.trackingId + '] partnerId [' + params.partnerId + '] url [' + url + '], ' + e.message);
            else
                response.log('beacon was sent, ignoring the error type [' + params.type + '] tracking id: [ '+ params.trackingId + '] partnerId [' + params.partnerId + '] url [' + url + ']');
        });
        request.on('socket', function (e) {
            response.log('Socket send beacon for type [' + params.type + '] tracking id: [ '+ params.trackingId + '] partnerId [' + params.partnerId + '] url [' + url + ']');
        });
    }

...
KalturaMainProcess.prototype.storeServerProcesses = function(processes){
	...
	this.cache.set(serverProcessesKey, processes, 60, function(){
		setInterval(function(){
			This.cache.set(serverProcessesKey, processes, 60);
		}, 55000);
	});
};
...
KalturaChildProcess.prototype.handleRequest = function(request, response) {
	...
	response.dir = function(msg){
		KalturaLogger.log('Request [' + response.requestId + ']');
		console.dir(msg);
	};
	response.error = function(msg){
		KalturaLogger.error('Request [' + response.requestId + '] ' + msg);
	};
	response.done = function(body){
		response.log(request.ip + ' "' + request.method + ' ' + request.url + ' ' + request.httpVersion + '" ' + response.statusCode);
		response.end(body);
	};
	...
	response.log('Service [' + urlInfo.service + '] action [' + urlInfo.action + ']');
	...
	if(!service){	
		...
		if(!service){
			response.error('Service [' + urlInfo.service + '] not found');
			return this.errorFileNotFound(response);
		}
		
		service.start();
		this.managers[urlInfo.service] = service;
	}
			
	if(!service[urlInfo.action] || !(typeof service[urlInfo.action] === 'function')){
		response.error('Action [' + urlInfo.service + '.' + urlInfo.action + '] not found');
		return this.errorFileNotFound(response);
	}

...
require('../../utils/KalturaConfig.js');
require('../../utils/KalturaLogger.js');
...
class KalturaVastParser
{
		...
		vastClient.get(vastUrl, customizedHeaders, timeout,
			function (vastObject)
			{
				if (KalturaVastParser._validate(vastObject))
					...
					KalturaLogger.error(`Invalid vast taken from url ${vastUrl}`);
	...
	{
		if (!vastObject) {
			KalturaLogger.error(`Vast object was returned null`);
			return true; //will pass through the vastObject
		}
		if (vastObject.ads.length == 0)
			return false;

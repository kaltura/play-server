...
KalturaClientBase.prototype.sendRequestHelper = function (options, requestIndex, onCompleteCallback, timeout, recursion = false) {
		...
		function (err , response, body){
			if (err)
			{
				if (err.code === 'ETIMEDOUT' && !recursion)
					return This.sendRequestHelper(options, requestIndex, onCompleteCallback,  timeout * 2, true);
				if (err.connect === true)
...
KalturaClientBase.prototype.doHttpRequest = function (callCompletedCallback, requestUrl, params, files) {
	...
	this.sendRequestHelper(options, requestIndex, callCompletedCallback, this.config.timeout);
};

...
KalturaClientBase.prototype.doQueue = function(callback){
	...
	if(Object.keys(files).length > 0)
		this.doHttpRequestWithFiles(callback, url, params, files);
	else
		this.doHttpRequestNoFiles(callback, url, params);
	this.resetRequest();
	return true;
};
...
KalturaClientBase.prototype.sendRequestHelperHttpModule = function (options, body, requestIndex, onCompleteCallback, timeout) {
	var This = this;
...
KalturaClientBase.prototype.sendRequestHelperRequestModule = function (options, requestIndex, onCompleteCallback, timeout, recursion = false) {
	...
	function returnError(err)
	{
		...
		return onCompleteCallback(null, err);
	}

	var This = this;
	if (timeout) {
		options.timeout = parseInt(timeout, 10) * 1000;
	}
	// adding pool for increasing the number of open sockets
	options.pool = {maxSockets: 40};
	request.post(options,
		function (err , response, body){
			if (err)
			{
				if (err.code === 'ETIMEDOUT' && !recursion)
					return This.sendRequestHelperRequestModule(options, requestIndex, onCompleteCallback,  options.timeout * 2, true);
				if (err.connect === true)
					This.error('Problem connecting to the API server');
			...
			if (response && response.statusCode === 200)
			{
				if (body)
				{
					...
					This.debug('Headers [' + requestIndex + ']: \n\t' + headers.join('\n\t'));
					This.debug('Response [' + requestIndex + ']: ' + util.inspect(body));
					if (This.config.format != KalturaClientBase.KALTURA_SERVICE_FORMAT_JSON)
					...
					else
					{
						...
						{
							This.log('XXX ' + util.inspect(body));
							onCompleteCallback(body);
						}
					...
					return returnError("Body was empty even when response was full");
			}
			else
			{
				if (response)
					return returnError("Bad response " + response.statusCode);
				else
					return returnError("No response ");
			}
		}
	);
}
 ...
 */
KalturaClientBase.prototype.doHttpRequestNoFiles = function (callCompletedCallback, requestUrl, params) {
	...
	this.sendRequestHelperRequestModule(options, requestIndex, callCompletedCallback, this.config.timeout);
};
 ...
 * @return array							 the results and errors inside an array.
 */
KalturaClientBase.prototype.doHttpRequestWithFiles = function (callCompletedCallback, requestUrl, params, files) {
	...
	this.log('ECK before calling request helper');
	this.sendRequestHelperHttpModule(options, multipartBody, requestIndex, callCompletedCallback, this.config.timeout);
};

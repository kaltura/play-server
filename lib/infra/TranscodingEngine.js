...
class TranscodingEngine
{
	...
	constructor(transcoderPath)
	{
		if (!transcoderPath)
		{
			if (KalturaConfig.config.bin.ffmpegPath)
				this._transcoderPath = KalturaConfig.config.bin.ffmpegPath;
			else
				this._transcoderPath = 'ffmpeg';
		}
		else
	...
	transcodeFile(commandLine, flavorId = null, adPath = null)
	{
		...
		return new Promise(function (resolve, reject)
		{
			...
			function callback(error, stdout, stderr)
			{
				...
				{
					KalturaTempFileHandler.onTranscodingFinished(adPath,
						function ()
						{

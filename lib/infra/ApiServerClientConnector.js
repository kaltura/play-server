...
class ApiServerClientConnector
{
			...
			function callback(result, err)
			{
				...
				else
				{
					if (ApiServerClientConnector._isValidResponse(result))
					{
                        ...
                        setTimeout(This._invalidateKs, KalturaConfig.config.client.ksExpiryTime*1000, This.client);
						KalturaLogger.log(`Setting Timeout for invalidating Ks - ${KalturaConfig.config.client.ksExpiryTime*1000}`);
	...
	_handleRequestPromise(params)
	{
		...
		return new Promise(function (resolve, reject)
		{
			function callback(result, err)
			{
				if (err)
				{
					KalturaLogger.log(`Got error when calling API server :${err}`);
					if (err.hasOwnProperty('objectType') && err.objectType === 'KalturaAPIException')
					{
						if (err.code === 'INVALID_KS' && err.args.KSID === 'EXPIRED')
				...
				else
				{
					...
					{
						KalturaLogger.log(`Failed to get response from API server :${result.message}`);
						reject(`KalturaAPIException ${result.message}`);
					}
				}
			}
	...
	static _getValueFromCache(cacheParams, passParams = null)
	{
		...
		return new Promise(function (resolve, reject)
		{
			...
			{
				KalturaLogger.log(`Could not get cache value for [${cacheKey}] due to cache error ${err}`);
				reject(passParams);
			}

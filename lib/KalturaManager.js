...
KalturaManager.prototype.getEntry = function(entryId, callback){
	...
	var This = this;
	var entryKey = KalturaCache.getEntry(entryId);
	
	var apiCall = function(){
		if(This.client == null)
			This.initClient(KalturaConfig.config.client);
		
		This.client.baseEntry.get(entryId, function(entry){
			KalturaCache.set(entryKey, entry, KalturaConfig.config.cache.entryRequired);
			callback(entry);
		});		
	};				
	KalturaCache.get(entryKey, function(entry){		
		if(!entry){
			apiCall();
		}
		else
...
KalturaManager.prototype.getMetadata = function(entryId, metadataProfileId, callback){
	...
	var This = this;
	var metadataKey = KalturaCache.getMetadata(entryId, metadataProfileId);
	...
	var apiCall = function(){
		...
		This.client.metadata.listAction(filter, pager, function(metadataList){
			...
			KalturaCache.set(metadataKey, metadata, KalturaConfig.config.cache.entryRequired);
			callback(metadata);
		});
	};
		
	KalturaCache.get(metadataKey, function(metadata){				
		if(!metadata){
			apiCall();
		}
		else

...
KalturaManager.prototype.getClient = function(config, callback){
	if (this.sessionReady && callback &&
		(this.sessionEpxiry && this.sessionStart && (parseInt(this.sessionEpxiry) + parseInt(this.sessionStart)) > parseInt(Date.now())) )
	{
		KalturaLogger.log('Client was already initialized');
		callback();
		return;
	}
	...
	if(!config.expiry){
		...
		ksTimer = config.expiry * 1000 - 1000;
	...
	this.client.session.start(function(ks){
		if(ks){
			...
			This.sessionEpxiry = ksTimer;
			setTimeout(function(){ This.getClient(config, callback); }, ksTimer, config, callback);
			if(callback){
				callback();
			}			
		}
...
KalturaManager.prototype.areValidApiResults = function(results, callers){
	...
	var isValid = true;
	for ( var i = 0; i < results & isValid ; i++ ) {

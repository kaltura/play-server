 ...
 * @param callback
 */
KalturaManager.prototype.getAndStoreUiConfConfig = function(uiConfId, entryId, partnerId, callback){
		...
		try{
			...
			This.client.uiConf.get(function(result){
				...
				if(!result){
					...
					var uiConfConfigfKey = KalturaCache.getKey(KalturaCache.UI_CONF_CONFIG_KEY_PREFIX, [uiConfId]);
					KalturaCache.set(uiConfConfigfKey, uiConfConfig, KalturaConfig.config.cache.masterManifest);
					if(callback){

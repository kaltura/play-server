...
KalturaManager.prototype.parsePlayServerParams = function(response, playServerParams, requiredParams){
	...
	var params = KalturaUtils.parseJSON(str);
	if (!params){
		response.error('Could not parse given params ' + str);
		...
		try{
			...
			This.client.uiConf.get(function(result){
				...
				if(!result){
					...
					var uiConfConfig = KalturaUiConfParser.parseUiConfConfig( KalturaUtils.parseJSON(result.config));

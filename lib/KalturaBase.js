...
KalturaLogger = {
	...
	debug: function(str, levelsUp){
		// TODO check configuration to know if debug enabled
		console.log(this.prefix(levelsUp) + ' DEBUG: ' + str);
	},
	
	log: function(str, levelsUp){
		console.log(this.prefix(levelsUp) + ' INFO: ' + str);
	},
	
	error: function(str, levelsUp){
		console.error(this.prefix(levelsUp) + ' ERROR: ' + str);
	},
	
	dir: function(object, levelsUp){
		console.log(this.prefix(levelsUp) + ' INFO: ');
		console.dir(object);
	}
};
	...
	get : function(key, callback, errorCallback) {
		this.server.get(key, function(err, data){
			if(err){
				...
				KalturaLogger.log('Cache.get [' + key + ']: OK', 2);
				if(callback)
					callback(data);
			}
		});
	...
	set : function(key, value, lifetime, callback, errorCallback) {
		this.server.set(key, value, lifetime, function(err){
			if(err){
				...
				KalturaLogger.log('Cache.set [' + key + ']: OK', 2);
				if(callback)
					callback();
			}
		});
	...
	touch : function(key, value, lifetime, callback, errorCallback) {
		var cacheTouchCallback = function(err){
			if(err){
				...
				KalturaLogger.log('Cache.touch [' + key + ']: OK', 2);
				if(callback)
					callback();
			}
		};
	...
	add : function(key, value, lifetime, callback, errorCallback) {
		this.server.add(key, value, lifetime, function(err){
			if(err){
				...
				KalturaLogger.log('Cache.add [' + key + ']: OK', 2);
				if(callback)
					callback();
			}
		});
	...
	append : function(key, value, callback, errorCallback) {
		this.server.append(key, value, function(err){
			if(err){
				...
				KalturaLogger.log('Cache.append [' + key + ']: OK', 2);
				if(callback)
					callback();
			}
		});
	...
	getMulti : function(keys, callback, errorCallback) {
		return this.server.getMulti(keys, function(err, data){
			if(err){
				...
				KalturaLogger.log('Cache.getMulti [' + key + ']: OK', 2);
				if(callback)
					callback(data);
			}
		});
	...
	del : function(key, callback, errorCallback) {
		return this.server.del(key, function(err){
			if(err){
				...
				KalturaLogger.log('Cache.del [' + key + ']: OK', 2);
				if(callback)
					callback();
			}
		});
	...
	replace : function(key, value, lifetime, callback, errorCallback) {
		return this.server.replace(key, value, lifetime, function(err){
			if(err){
				...
				KalturaLogger.log('Cache.replace [' + key + ']: OK', 2);
				if(callback)
					callback();
			}
		});

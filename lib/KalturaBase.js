...
KalturaBase.prototype = {
	...
	callPlayServerService : function(service, action, params, successCallback, failureCallback){
		...
		var playServerUrl = this.getPlayServerUrl(service, action, signedParams, null, KalturaConfig.config.cloud.internalDomain);
	...
	getPlayServerUrl : function(service, action, params, encryptedParams, domain){
		if(!domain)
		...
		if(encryptedParams){
			params.e = this.encrypt(encryptedParams);
		}

		...
		return new Promise(function (resolve, reject) {
			client.flavorAsset.getByEntryId(function (results) {
					if (results && results.code && results.message) {
						...
						let playManifest = PlayServerTestingHelper.serverHost + '/p/' + PlayServerTestingHelper.partnerId + '/sp/' + PlayServerTestingHelper.partnerId +
							'/playManifest/entryId/' + entry.id + '/flavorIds/' + flavor.id + '/format/applehttp/protocol/http/a.m3u8?uiconf=' + uiConfId;
			...
			child_process.exec('zbarimg ' + videoThumbDir + filename,
				(error, stdout, stderr) => {
					...
					if (result) {
						...
						console.warn(`Failed to read QR code from ${videoThumbDir + filename}`);

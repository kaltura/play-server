...
describe('testManifestLayout middle cue points and no cuepoints ', function() {
	...
	var vodData = new VodData(PARTNER_ID, FLAVOR_IDS, matchingEntry, middleCuePoints, flavorUrls);

	it('test createNoCuePointsManifestLayout', function () {
		var noAdsLayoutResult = kalturaLayoutManager.createNoCuePointsManifestLayout(vodData);
		var noCuePointsLayout = JSON.parse(noAdsLayoutResult);
	...
	it('test createFullManifestLayout', function () {
		var fullLayoutResult = kalturaLayoutManager.createFullManifestLayout(vodData);
		var fullLayout = JSON.parse(fullLayoutResult);
...
describe('testManifestLayout ending cue points', function() {
	...
	var vodData = new VodData(PARTNER_ID, FLAVOR_IDS, matchingEntry, endingCuePoints, flavorUrls);
...
describe('test filtering cue points', function() {
	it('test all cue points cases', function(){
		var cuePointsObject = JSON.parse(CUE_POINT_CASES);
		var filteredCuePoints = kalturaLayoutManager.getFilteredCuePoints(cuePointsObject);
...
describe('test matching of flavors and entry', function() {
	it('test expected success match ', function(){
		//var apiResults = JSON.parse(TWO_MIDDLE_CUE_POINTS_API_RESULTS);
		var vodData = new VodData(PARTNER_ID, FLAVOR_IDS, matchingEntry, middleCuePoints, flavorUrls);
		var isMatch = kalturaLayoutManager.isEntryFlavorsMatch(vodData);
		expect(isMatch).to.equal(true);
	});
	it('test expected failure match ', function(){
		//var apiResults = JSON.parse(FAULTY_ENTRY_API_RESULTS);
		var vodData = new VodData(PARTNER_ID, FLAVOR_IDS, nonMatchingEntry, middleCuePoints, flavorUrls);
		var isMatch = kalturaLayoutManager.isEntryFlavorsMatch(vodData);
...
describe('test beacon offset calculation', function() {
	it('test all types of beacons', function(){
		expect(kalturaLayoutManager.calculateBeaconOffset('impression', 0, 50)).to.equal(0);
		expect(kalturaLayoutManager.calculateBeaconOffset('impression', 5000, 50)).to.equal(5000);
		expect(kalturaLayoutManager.calculateBeaconOffset('start', 3, 50)).to.equal(3);
		expect(kalturaLayoutManager.calculateBeaconOffset('firstQuartile', 1000, 1000)).to.equal(1250);
		expect(kalturaLayoutManager.calculateBeaconOffset('firstQuartile', 100, 99)).to.equal(124);
		expect(kalturaLayoutManager.calculateBeaconOffset('midpoint', 200, 50)).to.equal(225);
		expect(kalturaLayoutManager.calculateBeaconOffset('midpoint', 200, 13)).to.equal(206);
		expect(kalturaLayoutManager.calculateBeaconOffset('thirdQuartile', 2000, 40)).to.equal(2030);
		expect(kalturaLayoutManager.calculateBeaconOffset('thirdQuartile', 200, 50)).to.equal(237);
		expect(kalturaLayoutManager.calculateBeaconOffset('complete', 3, 50)).to.equal(53);
